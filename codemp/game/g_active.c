...
qboolean WP_SaberStyleValidForSaber(saberInfo_t *saber1, saberInfo_t *saber2,
                                    int saberHolstered, int saberAnimLevel);
qboolean saberCheckKnockdown_DuelLoss(gentity_t *saberent,
                                      gentity_t *saberOwner, gentity_t *other);
...
void P_DamageFeedback(gentity_t *player) {
  ...
  if ((level.time > player->pain_debounce_time) &&
      !(player->flags & FL_GODMODE) && !(player->s.eFlags & EF_DEAD) &&
      (player->client->tempSpectate < level.time)) {

    // don't do more than two pain sounds a second
    // nmckenzie: also don't make him loud and whiny if he's only getting
    // nicked.
  ...
  if (waterlevel == 3) {
    ...
    if (ent->client->airOutTime < level.time) {
      ...
      if (ent->health > 0 && ent->client->tempSpectate < level.time) {
        ...
        G_Damage(ent, NULL, NULL, NULL, NULL, ent->damage, DAMAGE_NO_ARMOR,
                 MOD_WATER);
      }
    }
  } else {
...
void DoImpact(gentity_t *self, gentity_t *other, qboolean damageSelf) {
  ...
  if(pointcontents(self.absmax)==CONTENT_WATER)//FIXME: or other watertypes
          magnitude/=3;							//water
  absorbs 2/3 velocity

  if(self.classname=="barrel"&&self.aflag)//rolling barrels are made for
  impacts! magnitude*=3;

  if(self.frozen>0&&magnitude<300&&self.flags&FL_ONGROUND&&loser==world&&self.velocity_z<-20&&self.last_onground+0.3<time)
          magnitude=300;
  */
  if (other->material == MAT_GLASS || other->material == MAT_GLASS_METAL ||
  ...
  if (!self->client || self->client->ps.lastOnGround + 300 < level.time ||
      (self->client->ps.lastOnGround + 100 < level.time && easyBreakBrush)) {
    vec3_t dir1, dir2;
    float force = 0, dot;
    ...
    if (magnitude >= 100 && other->s.number < ENTITYNUM_WORLD) {
      ...
      if (VectorCompare(other->r.currentOrigin,
                        vec3_origin)) { // a brush with no origin
      ...
      if ((cont &
           CONTENTS_WATER)) //|| (self.classname=="barrel"&&self.aflag))//FIXME:
                            // or other watertypes
      {
      ...
      if ((force >= 1 && other->s.number >= MAX_CLIENTS) || force >= 10) {
        ...
        */
        if (other->r.svFlags & SVF_GLASS_BRUSH) {
          other->splashRadius =
              (float)(self->r.maxs[0] - self->r.mins[0]) / 4.0f;
        }
        if (other->takedamage) {
          G_Damage(other, self, self, velocity, self->r.currentOrigin, force,
                   DAMAGE_NO_ARMOR, MOD_CRUSH); // FIXME: MOD_IMPACT
        } else {
    ...
    if (damageSelf && self->takedamage) {
      // Now damage me
      // FIXME: more lenient falling damage, especially for when driving a
      // vehicle
      if (self->client &&
          self->client->ps.fd.forceJumpZStart) { // we were force-jumping
        if (self->r.currentOrigin[2] >=
            self->client->ps.fd.forceJumpZStart) { // we landed at same height
                                                   // or higher than we landed
          magnitude = 0;
      ...
      // if(self.classname!="monster_mezzoman"&&self.netname!="spider")//Cats
      // always land on their feet
      if ((magnitude >= 100 + self->health && self->s.number >= MAX_CLIENTS &&
           self->s.weapon != WP_SABER) ||
          (magnitude >= 700)) //&& self.safe_time < level.time ))//health here
                              // is used to simulate structural integrity
      {
        if ((self->s.weapon == WP_SABER) && self->client &&
            self->client->ps.groundEntityNum < ENTITYNUM_NONE &&
            magnitude < 1000) { // players and jedi take less impact damage
          // allow for some lenience on high falls
          magnitude /= 2;
          /*
          if ( self.absorb_time >= time )//crouching on impact absorbs 1/2 the
          damage
          {
                  magnitude/=2;
          }
        ...
        magnitude /= 40;
        magnitude = magnitude - force / 2; // If damage other, subtract half of
                                           // that damage off of own injury
        if (magnitude >= 1) {
          ...
          */
          /*
          if ( self.classname=="player_sheep "&& self.flags&FL_ONGROUND &&
          self.velocity_z > -50 ) return;
          */
          G_Damage(self, NULL, NULL, NULL, self->r.currentOrigin, magnitude / 2,
                   DAMAGE_NO_ARMOR, MOD_FALLING); // FIXME: MOD_IMPACT
        }
      }
    }
...
void Client_CheckImpactBBrush(gentity_t *self, gentity_t *other) {
  ...
  if (!self || !self->inuse || !self->client ||
      ...
      self->client->sess.sessionTeam ==
          TEAM_SPECTATOR) { // hmm.. let's not let spectators ram into
                            // breakables.
    return;
  }
  ...
  */
  ...
  if (other->material == MAT_GLASS || other->material == MAT_GLASS_METAL ||
      ...
      (other->r.svFlags & SVF_GLASS_BRUSH)) { // clients only do impact damage
                                              // against easy-break breakables
...
void G_SetClientSound(gentity_t *ent) {
  if (ent->client && ent->client->isHacking) { // loop hacking sound
    ...
    ent->s.loopIsSoundset = qfalse;
             ...
             ent->client->isMedHealed > level.time) { // loop healing sound
    ent->client->ps.loopSound = level.snd_medHealed;
    ent->s.loopIsSoundset = qfalse;
  } else if (ent->client &&
             ent->client->isMedSupplied > level.time) { // loop supplying sound
    ent->client->ps.loopSound = level.snd_medSupplied;
    ent->s.loopIsSoundset = qfalse;
  } else if (ent->client && ent->waterlevel &&
...
static void SV_PMTrace(trace_t *results, const vec3_t start, const vec3_t mins,
                       const vec3_t maxs, const vec3_t end, int passEntityNum,
                       int contentMask) {
  trap->Trace(results, start, mins, maxs, end, passEntityNum, contentMask,
              qfalse, 0, 10);
}
  ...
  if (client->tempSpectate < level.time) {
    ...
    if ((client->buttons & BUTTON_ATTACK) &&
      ...
      Cmd_FollowCycle_f(ent, 1);
             ...
             (client->buttons & BUTTON_ALT_ATTACK) &&
             !(client->oldbuttons & BUTTON_ALT_ATTACK))
      Cmd_FollowCycle_f(ent, -1);

    if (client->sess.spectatorState == SPECTATOR_FOLLOW &&
        (ucmd->upmove > 0)) { // jump now removes you from follow mode
      StopFollowing(ent);
    }
  }
}
...
qboolean ClientInactivityTimer(gclient_t *client) {
  if (!g_inactivity.integer) {
    ...
    if (level.time > client->inactivityTime - 10000 &&
      ...
      trap->SendServerCommand(client - level.clients,
...
extern void NPC_SetAnim(gentity_t *ent, int setAnimParts, int anim,
                        int setAnimFlags);
void G_VehicleAttachDroidUnit(gentity_t *vehEnt) {
  if (vehEnt && vehEnt->m_pVehicle &&
      vehEnt->m_pVehicle->m_pDroidUnit != NULL) {
    gentity_t *droidEnt = (gentity_t *)vehEnt->m_pVehicle->m_pDroidUnit;
    mdxaBone_t boltMatrix;
    vec3_t fwd;

    trap->G2API_GetBoltMatrix(vehEnt->ghoul2, 0,
                              vehEnt->m_pVehicle->m_iDroidUnitTag, &boltMatrix,
                              vehEnt->r.currentAngles, vehEnt->r.currentOrigin,
    ...
    if (droidEnt->NPC) {
      NPC_SetAnim(droidEnt, SETANIM_BOTH, BOTH_STAND2,
...
void G_CheapWeaponFire(int entNum, int ev) {
  ...
  case EV_FIRE_WEAPON:
    if (ent->m_pVehicle &&
        ent->m_pVehicle->m_pVehicleInfo->type == VH_SPEEDER && ent->client &&
        ent->client->ps.m_iVehicleNum) { // a speeder with a pilot
      gentity_t *rider = &g_entities[ent->client->ps.m_iVehicleNum - 1];
      if (rider->inuse && rider->client) { // pilot is valid...
        if (rider->client->ps.weapon != WP_MELEE &&
            (rider->client->ps.weapon != WP_SABER ||
             !BG_SabersOff(&rider->client->ps))) { // can only attack on speeder
                                                   // when using melee or when
                                                   // saber is holstered
          break;
        }
      }
...
void ClientEvents(gentity_t *ent, int oldEventSequence) {
  ...
  for (i = oldEventSequence; i < client->ps.eventSequence; i++) {
    ...
    case EV_ROLL: {
      ...
      if (knockDownage) {
        damage =
            delta * 1; // you suffer for falling unprepared. A lot. Makes throws
                       // and things useful, and more realistic I suppose.
      } else {
        if (level.gametype == GT_SIEGE && delta > 60) { // longer falls hurt
                                                        // more
      ...
      G_Damage(ent, NULL, NULL, NULL, NULL, damage, DAMAGE_NO_ARMOR,
               MOD_FALLING);
    ...
    // rww - Note that these must be in the same order (ITEM#-wise) as they are
    // in holdable_t
...
static const float maxJediMasterDistance =
    2500.0f * 2500.0f; // x^2, optimisation
static const float maxJediMasterFOV = 100.0f;
static const float maxForceSightDistance =
    Square(1500.0f) * 1500.0f; // x^2, optimisation
...
void G_UpdateClientBroadcasts(gentity_t *self) {
  ...
  // if we do not wish to be sent to any particular entity, we must set the
  // broadcastClients bit field and the
  ...
  for (i = 0, other = g_entities; i < MAX_CLIENTS; i++, other++) {
    ...
    if (level.gametype == GT_JEDIMASTER && self->client->ps.isJediMaster) {
      if (dist < maxJediMasterDistance &&
          InFieldOfVision(other->client->ps.viewangles, maxJediMasterFOV,
                          angles)) {
        send = qtrue;
      }
    }

    // broadcast this client to everyone using force sight if we are in
    // distance/field of view
    if ((other->client->ps.fd.forcePowersActive & (1 << FP_SEE))) {
      if (dist < maxForceSightDistance &&
          InFieldOfVision(other->client->ps.viewangles, maxForceSightFOV,
                          angles)) {
        send = qtrue;
      }
    }
...
void G_AddPushVecToUcmd(gentity_t *self, usercmd_t *ucmd) {
  ...
  VectorScale(forward, ucmd->forwardmove / 127.0f * self->client->ps.speed,
              moveDir);
  VectorMA(moveDir, ucmd->rightmove / 127.0f * self->client->ps.speed, right,
           moveDir);
...
qboolean G_StandingAnim(
    int anim) { // NOTE: does not check idles or special (cinematic) stands
  switch (anim) {
  case BOTH_STAND1:
  case BOTH_STAND2:
  case BOTH_STAND3:
...
void G_CheckClientIdle(gentity_t *ent, usercmd_t *ucmd) {
  ...
  VectorSubtract(ent->client->ps.viewangles, ent->client->idleViewAngles,
                 viewChange);
  if (!VectorCompare(vec3_origin, ent->client->ps.velocity) || actionPressed ||
      ucmd->forwardmove || ucmd->rightmove || ucmd->upmove ||
      !G_StandingAnim(ent->client->ps.legsAnim) ||
      (ent->health + ent->client->ps.stats[STAT_ARMOR]) !=
          ent->client->idleHealth ||
      VectorLength(viewChange) > 10 || ent->client->ps.legsTimer > 0 ||
      ent->client->ps.torsoTimer > 0 || ent->client->ps.weaponTime > 0 ||
      ...
      (ent->client->ps.weapon != ent->client->pers.cmd.weapon &&
       ent->s.eType != ET_NPC)) { // FIXME: also check for turning?
  ...
  } else if (level.time - ent->client->idleTime >
             5000) { // been idle for 5 seconds
    int idleAnim = -1;
    switch (ent->client->ps.legsAnim) {
    case BOTH_STAND1:
    ...
    case BOTH_STAND2:
      idleAnim =
          BOTH_STAND2IDLE1; // Q_irand(BOTH_STAND2IDLE1,BOTH_STAND2IDLE2);
      break;
    case BOTH_STAND3:
    ...
    if (/*PM_HasAnimation( ent, idleAnim )*/ idleAnim > 0 &&
        idleAnim < MAX_ANIMATIONS) {
      G_SetAnim(ent, ucmd, SETANIM_BOTH, idleAnim,
                SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD, 0);

      // don't idle again after this anim for a while
      // ent->client->idleTime = level.time + PM_AnimLength(
      // ent->client->clientInfo.animFileIndex, (animNumber_t)idleAnim ) +
      // Q_irand( 0, 2000 );
      ent->client->idleTime =
          level.time + ent->client->ps.legsTimer + Q_irand(0, 2000);
    }
  }
}
...
void NPC_Accelerate(gentity_t *ent, qboolean fullWalkAcc, qboolean fullRunAcc) {
  ...
  if (!ent->NPC->stats
           .acceleration) { // No acceleration means just start and stop
    ent->NPC->currentSpeed = ent->NPC->desiredSpeed;
  }
  // FIXME:  in cinematics always accel/decel?
  else if (ent->NPC->desiredSpeed <=
           ent->NPC->stats.walkSpeed) { // Only accelerate if at walkSpeeds
      ...
      ent->NPC->currentSpeed = ent->NPC->desiredSpeed;
               ...
               ent->NPC->desiredSpeed <
                   ent->NPC->currentSpeed -
                       ent->NPC->stats
                           .acceleration) { // decelerate even when walking
      ent->NPC->currentSpeed -= ent->NPC->stats.acceleration;
    } else if (ent->NPC->desiredSpeed <
               ent->NPC->currentSpeed) { // stop on a dime
      ent->NPC->currentSpeed = ent->NPC->desiredSpeed;
    }
  ...
  {      // Only decelerate if at runSpeeds
    if (fullRunAcc &&
        ent->NPC->desiredSpeed >
            ent->NPC->currentSpeed +
                ent->NPC->stats.acceleration) { // Accelerate to runspeed
      // ent->client->ps.friction = 0;
      ent->NPC->currentSpeed += ent->NPC->stats.acceleration;
    } else if (ent->NPC->desiredSpeed >
               ent->NPC->currentSpeed) { // accelerate instantly
      // ent->client->ps.friction = 0;
      ent->NPC->currentSpeed = ent->NPC->desiredSpeed;
    } else if (fullRunAcc &&
               ent->NPC->desiredSpeed <
...
static int NPC_GetWalkSpeed(gentity_t *ent) {
  ...
  case NPCTEAM_PLAYER: // To shutup compiler, will add entries later (this is
                       // stub code)
  default:
...
static int NPC_GetRunSpeed(gentity_t *ent) {
  ...
  // team no longer indicates species/race.  Use NPC_class to adjust speed for
  // specific npc types
  ...
  default:
    runSpeed = ent->NPC->stats.runSpeed *
               1.3f; // rww - seems to slow in MP for some reason.
    break;
  }
...
// Seems like a slightly less than ideal method for this, could it be done on
// the client?
extern qboolean FlyingCreature(gentity_t *ent);
void G_CheckMovingLoopingSounds(gentity_t *ent, usercmd_t *ucmd) {
  if (ent->client) {
    if ((ent->NPC &&
         !VectorCompare(vec3_origin,
                        ent->client->ps.moveDir)) // moving using moveDir
        ...
        || (FlyingCreature(ent) &&
            ...
            ent->health > 0)) // flier using velocity to move
    {
      switch (ent->client->NPC_class) {
      case CLASS_R2D2:
...
void G_HeldByMonster(gentity_t *ent, usercmd_t *ucmd) {
  if (ent && ent->client &&
      ent->client->ps
          .hasLookTarget) // NOTE: lookTarget is an entity number, so this
                          // presumes that client 0 is NOT a Rancor...
  {
    ...
    if (monster && monster->client) {
      ...
      if (monster->s.NPC_class ==
          CLASS_RANCOR) { // only possibility right now, may add Wampa and Sand
                          // Creature later
        BG_AttachToRancor(
            monster->ghoul2, // ghoul2 info
            monster->r.currentAngles[YAW], monster->r.currentOrigin, level.time,
            NULL, monster->modelScale,
            (monster->client->ps.eFlags2 & EF2_GENERIC_NPC_FLAG),
            ent->client->ps.origin, ent->client->ps.viewangles, NULL);
      }
...
void G_SetTauntAnim(gentity_t *ent, int taunt) {
  if (ent->client->pers.cmd.upmove || ent->client->pers.cmd.forwardmove ||
      ent->client->pers.cmd.rightmove) { // hack, don't do while moving
    return;
  }
  if (taunt != TAUNT_TAUNT) { // normal taunt always allowed
    if (level.gametype != GT_DUEL &&
        level.gametype != GT_POWERDUEL) { // no taunts unless in Duel
      return;
    }
  }
  ...
  if (ent->client->ps.torsoTimer < 1 &&
    ...
    if (anim != -1) {
      if (ent->client->ps.groundEntityNum != ENTITYNUM_NONE) {
        ...
        ent->client->ps.forceHandExtendTime =
            level.time + BG_AnimLength(ent->localAnimIndex, (animNumber_t)anim);
      }
...
void ClientThink_real(gentity_t *ent) {
  ...
  if (ent->s.number < MAX_CLIENTS &&
      ent->client->ps.m_iVehicleNum) { // driving a vehicle
    if (g_entities[ent->client->ps.m_iVehicleNum].client) {
      gentity_t *veh = &g_entities[ent->client->ps.m_iVehicleNum];

      if (veh->m_pVehicle &&
          veh->m_pVehicle->m_pPilot ==
              (bgEntity_t *)ent) { // only take input from the pilot...
        veh->client->ps.commandTime = ent->client->ps.commandTime;
        memcpy(&veh->m_pVehicle->m_ucmd, &ent->client->pers.cmd,
               sizeof(usercmd_t));
        if (veh->m_pVehicle->m_ucmd.buttons &
            BUTTON_TALK) { // forced input if "chat bubble" is up
  ...
  if (!isFollowing) {
    if (level.gametype == GT_SIEGE && client->siegeClass != -1 &&
        bgSiegeClasses[client->siegeClass]
            .saberStance) { // the class says we have to use this stance set.
      if (!(bgSiegeClasses[client->siegeClass].saberStance &
            (1 << client->ps.fd.saberAnimLevel))) { // the current stance is not
                                                    // in the bitmask, so find
                                                    // the first one that is.
        int i = SS_FAST;

        while (i < SS_NUM_SABER_STYLES) {
          if (bgSiegeClasses[client->siegeClass].saberStance & (1 << i)) {
            if (i == SS_DUAL && client->ps.saberHolstered ==
                                    1) { // one saber should be off, adjust
                                         // saberAnimLevel accordinly
            ...
            } else if (i == SS_STAFF && client->ps.saberHolstered == 1 &&
                       client->saber[0].singleBladeStyle !=
                           SS_NONE) { // one saber or blade should be off,
                                      // adjust saberAnimLevel accordinly
              client->ps.fd.saberAnimLevelBase = i;
          ...
          i++;
               ...
               client->saber[1]
                   .model[0]) { // with two sabs always use akimbo style
      if (client->ps.saberHolstered ==
          1) { // one saber should be off, adjust saberAnimLevel accordinly
        client->ps.fd.saberAnimLevelBase = SS_DUAL;
        client->ps.fd.saberAnimLevel = SS_FAST;
        client->ps.fd.saberDrawAnimLevel = client->ps.fd.saberAnimLevel;
      } else {
        if (!WP_SaberStyleValidForSaber(
                &client->saber[0], &client->saber[1], client->ps.saberHolstered,
                client->ps.fd
                    .saberAnimLevel)) { // only use dual style if the style
                                        // we're trying to use isn't valid
          client->ps.fd.saberAnimLevelBase = client->ps.fd.saberAnimLevel =
              SS_DUAL;
        }
        client->ps.fd.saberDrawAnimLevel = client->ps.fd.saberAnimLevel;
      ...
      if (client->saber[0].stylesLearned ==
          (1 << SS_STAFF)) { // then *always* use the staff style
      ...
      if (client->ps.fd.saberAnimLevelBase == SS_STAFF) { // using staff style
        if (client->ps.saberHolstered == 1 &&
            client->saber[0].singleBladeStyle !=
                SS_NONE) { // one blade should be off, adjust saberAnimLevel
                           // accordinly
          client->ps.fd.saberAnimLevel = client->saber[0].singleBladeStyle;
          client->ps.fd.saberDrawAnimLevel = client->ps.fd.saberAnimLevel;
        } else {
  ...
  if (level.intermissiontime) {
    if (ent->s.number < MAX_CLIENTS ||
        client->NPC_class ==
            CLASS_VEHICLE) { // players and vehicles do nothing in intermissions
...
#define MIN_NPC_SPEED 16.0f
  ...
  if (client->bodyGrabIndex != ENTITYNUM_NONE) {
    ...
    if (!grabbed->inuse || grabbed->s.eType != ET_BODY ||
      ...
      trap->G2API_GetBoltMatrix(
          ent->ghoul2, 0, 0, &rhMat, tAng, ent->client->ps.origin, level.time,
  ...
  if (ent->NPC &&
      ent->s.NPC_class != CLASS_VEHICLE) // vehicles manage their own speed
  {
    ...
    if (ent->NPC->combatMove == qfalse) {
      ...
      if (1) { // Not leaning
        qboolean Flying =
            (ucmd->upmove &&
             (ent->client->ps.eFlags2 &
              EF2_FLYING)); // ent->client->moveType == MT_FLYSWIM);
        ...
        if (ucmd->forwardmove || ucmd->rightmove || Flying) {
          ...
          { // In - Formation NPCs set thier desiredSpeed themselves
            if (ucmd->buttons & BUTTON_WALKING) {
              ent->NPC->desiredSpeed =
                  NPC_GetWalkSpeed(ent); // ent->NPC->stats.walkSpeed;
            } else                       // running
            {
              ent->NPC->desiredSpeed =
                  NPC_GetRunSpeed(ent); // ent->NPC->stats.runSpeed;
            }

            if (ent->NPC->currentSpeed >= 80 &&
                !controlledByPlayer) { // At higher speeds, need to slow down
                                       // close to stuff
              // Slow down as you approach your goal
              if (ent->NPC->distToGoal < SLOWDOWN_DIST &&
                  !(ent->NPC->aiFlags & NPCAI_NO_SLOWDOWN)) // 128
              {
                if (ent->NPC->desiredSpeed > MIN_NPC_SPEED) {
                  float slowdownSpeed = ((float)ent->NPC->desiredSpeed) *
                                        ent->NPC->distToGoal / SLOWDOWN_DIST;

                  ent->NPC->desiredSpeed = ceil(slowdownSpeed);
                  if (ent->NPC->desiredSpeed <
                      MIN_NPC_SPEED) { // don't slow down too much
        ...
        if (ent->NPC->currentSpeed <= 24 &&
            ent->NPC->desiredSpeed <
                ent->NPC->currentSpeed) { // No-one walks this slow
          client->ps.speed = ent->NPC->currentSpeed = 0; // Full stop
          ucmd->forwardmove = 0;
          ucmd->rightmove = 0;
        } else {
          if (ent->NPC->currentSpeed <=
              ent->NPC->stats.walkSpeed) { // Play the walkanim
          ...
          if (ent->NPC->currentSpeed > 0) { // We should be moving
            if (Climbing || Flying) {
              if (!ucmd->upmove) { // We need to force them to take a couple
                                   // more steps until stopped
                ucmd->upmove = ent->NPC->last_ucmd.upmove; // was last_upmove;
              }
            } else if (!ucmd->forwardmove &&
                       !ucmd->rightmove) { // We need to force them to take a
                                           // couple more steps until stopped
              ucmd->forwardmove =
                  ent->NPC->last_ucmd.forwardmove; // was last_forwardmove;
              ucmd->rightmove =
                  ent->NPC->last_ucmd.rightmove; // was last_rightmove;
            }
          }

          client->ps.speed = ent->NPC->currentSpeed;
          //	if ( player && player->client && player->client->ps.viewEntity
          //== ent->s.number )
          //	{
          //	}
          //	else
          ...
          if (1) {
            ...
            float turndelta = 0;
            // if the NPC is locked into a Yaw, we want to check the
            // lockedDesiredYaw...otherwise the NPC can't walk backwards,
            // because it always thinks it trying to turn according to
            // desiredYaw
            // if( client->renderInfo.renderFlags & RF_LOCKEDANGLE ) // yeah I
            // know the RF_ flag is a pretty ugly hack...
            if (0) // rwwFIXMEFIXME: ...
            {
              turndelta = (180 - fabs(AngleDelta(ent->r.currentAngles[YAW],
                                                 ent->NPC->lockedDesiredYaw))) /
                          180;
            } else {
              turndelta = (180 - fabs(AngleDelta(ent->r.currentAngles[YAW],
                                                 ent->NPC->desiredYaw))) /
            ...
            if (turndelta < 0.75f) {
              client->ps.speed = 0;
            } else if (ent->NPC->distToGoal < 100 &&
                       turndelta < 1.0) { // Turn is greater than 45 degrees or
                                          // closer than 100 to goal
      ...
      ent->NPC->desiredSpeed = (ucmd->buttons & BUTTON_WALKING)
    ...
    if (ucmd->buttons &
        BUTTON_WALKING) { // sort of a hack I guess since MP handles walking
                          // differently from SP (has some proxy cheat
                          // prevention methods)
      /*
      ...
      */
      ...
      // ent->client->ps.speed = ent->client->ps.basespeed = NPC_GetRunSpeed(
      // ent );
    }
    client->ps.basespeed = client->ps.speed;
  } else if (!client->ps.m_iVehicleNum &&
             (!ent->NPC ||
              ent->s.NPC_class != CLASS_VEHICLE)) // if riding a vehicle it will
                                                  // manage our speed and such
  {
    // set speed
    ...
    if (client->bodyGrabIndex !=
        ENTITYNUM_NONE) { // can't go nearly as fast when dragging a body around
  ...
  if (!ent->NPC ||
      !(ent->NPC->aiFlags & NPCAI_CUSTOM_GRAVITY)) { // use global gravity
    if (ent->NPC && ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle &&
        ent->m_pVehicle->m_pVehicleInfo->gravity) { // use custom veh gravity
      client->ps.gravity = ent->m_pVehicle->m_pVehicleInfo->gravity;
    } else {
      if (ent->client->inSpaceIndex &&
          ent->client->inSpaceIndex !=
              ENTITYNUM_NONE) { // in space, so no gravity...
  ...
  if (ent->client->ps.duelInProgress) {
    ...
    // Keep the time updated, so once this duel ends this player can't engage in
    // a duel for another 10 seconds. This will give other people a chance to
    // engage in duels in case this player wants to engage again right after
    ...
    if (ent->client->ps.duelTime < level.time) {
      ...
      if (ent->client->ps.weapon == WP_SABER &&
          ent->client->ps.saberHolstered && ent->client->ps.duelTime) {
        ent->client->ps.saberHolstered = 0;
    ...
    if (!duelAgainst || !duelAgainst->client || !duelAgainst->inuse ||
      ...
      G_AddEvent(ent, EV_PRIVATE_DUEL, 0);
               ...
               duelAgainst->client->ps.stats[STAT_HEALTH] < 1) {
      ent->client->ps.duelInProgress = 0;
      ...
      if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0) {
        ...
        if (g_spawnInvulnerability.integer) {
          ...
          ent->client->invulnerableTimer =
              level.time + g_spawnInvulnerability.integer;
        }
      }

      /*
      trap->SendServerCommand( ent-g_entities, va("print \"%s %s\n\"",
      ent->client->pers.netname, G_GetStringEdString("MP_SVGAME",
      "PLDUELWINNER")) ); trap->SendServerCommand( duelAgainst-g_entities,
      va("print \"%s %s\n\"", ent->client->pers.netname,
      G_GetStringEdString("MP_SVGAME", "PLDUELWINNER")) );
      */
      // Private duel announcements are now made globally because we only want
      // one duel at a time.
      if (ent->health > 0 && ent->client->ps.stats[STAT_HEALTH] > 0) {
        ...
        trap->SendServerCommand(-1, va("ranked_duel_end %d %d 0\n", ent->s.number, duelAgainst->s.number));
      } else { // it was a draw, because we both managed to die in the same
               // frame
        trap->SendServerCommand(
            -1,
            va("cp \"%s\n\"", G_GetStringEdString("MP_SVGAME", "PLDUELTIE")));
        trap->SendServerCommand(-1, va("ranked_duel_end %d %d 1\n", ent->s.number, duelAgainst->s.number));
      }
    } else {
      vec3_t vSub;
      float subLen = 0;

      VectorSubtract(ent->client->ps.origin, duelAgainst->client->ps.origin,
                     vSub);
      ...
      if (subLen >= 1024) {
        ...
        trap->SendServerCommand(-1, va("ranked_duel_stop %d %d\n", ent->s.number, duelAgainst->s.number));
  ...
  if (ent->client->beingThrown > level.time) {
    ...
    if (!thrower->inuse || !thrower->client || thrower->health < 1 ||
...
#endif
      {
        ...
        VectorSubtract(thrower->client->ps.origin, ent->client->ps.origin,
                       entDir);
        ...
        if (VectorLength(vDif) > 32.0f &&
            (thrower->client->doingThrow - level.time) <
                4500) { // the hand is too far away, and can no longer hold onto
                        // us, so escape.
          ent->client->ps.heldByClient = 0;
          ent->client->beingThrown = 0;
          thrower->client->doingThrow = 0;
        ...
        } else if ((client->beingThrown - level.time) <
                   4000) { // step into the next part of the throw, and go
                           // flying back
          float vScale = 400.0f;
        ...
        } else { // see if we can move to be next to the hand.. if it's not
          ...
          trap->Trace(&tr, intendedOrigin, ent->r.mins, ent->r.maxs,
                      intendedOrigin, ent->s.number, ent->clipmask, qfalse, 0,
                      0);
          trap->Trace(&tr2, ent->client->ps.origin, ent->r.mins, ent->r.maxs,
                      intendedOrigin, ent->s.number, CONTENTS_SOLID, qfalse, 0,
                      0);
          ...
          } else { // if the guy can't be put here then it's time to break the
                   // throw off.
            ent->client->ps.heldByClient = 0;
            ent->client->beingThrown = 0;
            thrower->client->doingThrow = 0;
  ...
  */
  ...
  // Will probably never need this again, since we have g2 properly serverside
  ...
  */
  ...
  if (ent->client && ent->client->ps.fallingToDeath &&
    ...
    if (ent->health > 0) {
      ...
      G_Damage(ent, otherKiller, otherKiller, NULL, ent->client->ps.origin,
               9999, DAMAGE_NO_PROTECTION, MOD_FALLING);
      ...
      G_MuteSound(ent->s.number,
                  CHAN_VOICE); // stop screaming, because you are dead!
    }
  }
  ...
  //	WP_SaberPositionUpdate(ent, ucmd); //check the server-side saber point,
  // do apprioriate server-side actions (effects are cs-only)
  ...
  // FIXME: need to do this before check to avoid walls and cliffs (or just
  // cliffs?)
...
#endif
      if (ent->ghoul2) {
    if (ent->localAnimIndex >
        1) { // if it isn't humanoid then we will be having none of this.
      pmove.ghoul2 = NULL;
    } else {
  ...
  // I'll just do this every frame in case the scale changes in realtime (don't
  // need to update the g2 inst for that)
  ...
  level.time - level.intermissionQueued <= 2500 ) { trap->SendConsoleCommand(
  EXEC_APPEND, "centerview\n");
                  }
  ...
  */
  ...
  if (ent->client->ps.saberLockTime > level.time) {
    ...
    if (ent->client->ps.saberLockHitCheckTime <
        level.time) { // have moved to next frame since last lock push
      ent->client->ps.saberLockHitCheckTime =
          level.time; // so we don't push more than once per server frame
      if ((ent->client->buttons & BUTTON_ATTACK) &&
          !(ent->client->oldbuttons & BUTTON_ATTACK)) {
        if (ent->client->ps.saberLockHitIncrementTime <
            level.time) { // have moved to next frame since last saberlock
                          // attack button press
          int lockHits = 0;
          ent->client->ps.saberLockHitIncrementTime =
              level.time; // so we don't register an attack key press more than
                          // once per server frame
          // NOTE: FP_SABER_OFFENSE level already taken into account in
          // PM_SaberLocked
          if ((ent->client->ps.fd.forcePowersActive &
               (1 << FP_RAGE))) { // raging: push harder
    ...
    if ((pmove.cmd.generic_cmd == GENCMD_ENGAGE_DUEL) &&
        (level.gametype == GT_DUEL ||
         level.gametype ==
             GT_POWERDUEL)) { // already in a duel, make it a taunt command
  ...
  if (ent->s.number >= MAX_CLIENTS) {
    ...
    if (ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle) {
      if (ent->m_pVehicle->m_pPilot) { // vehicles want to use their last pilot
                                       // ucmd I guess
        if ((level.time - ent->m_pVehicle->m_ucmd.serverTime) >
            2000) { // Previous owner disconnected, maybe
        ...
        pmove.cmd.buttons = (g_entities[ent->m_pVehicle->m_pPilot->s.number]
                                 .client->pers.cmd.buttons &
                             (BUTTON_ATTACK | BUTTON_ALT_ATTACK));
      }
      if (ent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER) {
        if (ent->client->ps.groundEntityNum !=
            ENTITYNUM_NONE) { // ATST crushes anything underneath it
          gentity_t *under = &g_entities[ent->client->ps.groundEntityNum];
          if (under && under->health && under->takedamage) {
            vec3_t down = {0, 0, -1};
            // FIXME: we'll be doing traces down from each foot, so we'll have a
            // real impact origin
            G_Damage(under, ent, ent, down, under->r.currentOrigin, 100, 0,
                     MOD_CRUSH);
          }
        }
      }
  ...
  if (pmove.checkDuelLoss) {
    if (pmove.checkDuelLoss > 0 &&
        (pmove.checkDuelLoss <= MAX_CLIENTS ||
         (pmove.checkDuelLoss < (MAX_GENTITIES - 1) &&
          g_entities[pmove.checkDuelLoss - 1].s.eType == ET_NPC))) {
      ...
      if (clientLost && clientLost->inuse && clientLost->client &&
        ...
        vec3_t attDir;
        VectorSubtract(ent->client->ps.origin, clientLost->client->ps.origin,
                       attDir);
        ...
        gGAvoidDismember = 1;
        G_Damage(clientLost, ent, ent, attDir, clientLost->client->ps.origin,
                 9999, DAMAGE_NO_PROTECTION, MOD_SABER);

        if (clientLost->health < 1) {
          gGAvoidDismember = 2;
          G_CheckForDismemberment(clientLost, ent,
                                  clientLost->client->ps.origin, 999,
                                  (clientLost->client->ps.legsAnim), qfalse);
        }

        gGAvoidDismember = 0;
      } else if (clientLost && clientLost->inuse && clientLost->client &&
                 clientLost->client->ps.forceHandExtend !=
                     HANDEXTEND_KNOCKDOWN &&
                 clientLost->client->ps
                     .saberEntityNum) { // if we didn't knock down it was a
                                        // circle lock. So as punishment, make
                                        // them lose their saber and go into a
                                        // proper anim
        saberCheckKnockdown_DuelLoss(
            &g_entities[clientLost->client->ps.saberEntityNum], clientLost,
            ent);
      }
    }
  ...
  if (pmove.cmd.generic_cmd &&
      (pmove.cmd.generic_cmd != ent->client->lastGenCmd ||
       ent->client->lastGenCmdTime < level.time)) {
    ent->client->lastGenCmd = pmove.cmd.generic_cmd;
    if (pmove.cmd.generic_cmd != GENCMD_FORCE_THROW &&
        pmove.cmd.generic_cmd !=
            GENCMD_FORCE_PULL) { // these are the only two where you wouldn't
                                 // care about a delay between
      ent->client->lastGenCmdTime =
          level.time +
          300; // default 100ms debounce between issuing the same command.
    }
    ...
    case GENCMD_ENGAGE_DUEL:
      if (level.gametype == GT_DUEL ||
          level.gametype ==
              GT_POWERDUEL) { // already in a duel, made it a taunt command
      } else {
  ...
  if (g_smoothClients.integer) {
    BG_PlayerStateToEntityStateExtraPolate(&ent->client->ps, &ent->s,
                                           ent->client->ps.commandTime, qfalse);
    // rww - 12-03-02 - Don't snap the origin of players! It screws prediction
    // all up.
  } else {
  ...
  if (ent->s.eType != ET_NPC || ent->s.NPC_class != CLASS_VEHICLE ||
      !ent->m_pVehicle ||
      !ent->m_pVehicle
           ->m_iRemovedSurfaces) { // let vehicles that are getting broken apart
                                   // do their own crazy sizing stuff
  ...
  if ((ent->client->pers.cmd.buttons & BUTTON_USE) &&
      ent->client->ps.useDelay < level.time) {
    TryUse(ent);
  ...
  if (client->ps.forceKickFlip) {
    ...
    if (faceKicked && faceKicked->client &&
        ...
        (!faceKicked->client->ps.duelInProgress ||
         faceKicked->client->ps.duelIndex == ent->s.number) &&
        (!ent->client->ps.duelInProgress ||
         ent->client->ps.duelIndex == faceKicked->s.number)) {
      if (faceKicked && faceKicked->client && faceKicked->health &&
          faceKicked->takedamage) { // push them away and do pain
        vec3_t oppDir;
        ...
        G_Damage(faceKicked, ent, ent, oppDir, client->ps.origin, strength,
                 DAMAGE_NO_ARMOR, MOD_MELEE);
        ...
        if (faceKicked->client->ps.weapon != WP_SABER ||
          ...
          if (faceKicked->health > 0 &&
            ...
            if (BG_KnockDownable(&faceKicked->client->ps) &&
                Q_irand(1, 10) <= 3) { // only actually knock over sometimes,
                                       // but always do velocity hit
              faceKicked->client->ps.forceHandExtend = HANDEXTEND_KNOCKDOWN;
              faceKicked->client->ps.forceHandExtendTime = level.time + 1100;
              faceKicked->client->ps.forceDodgeAnim =
                  0; // this toggles between 1 and 0, when it's 1 we should play
                     // the get up anim
            }
  ...
  if (client->ps.stats[STAT_HEALTH] <= 0 &&
      !(client->ps.eFlags2 &
        EF2_HELD_BY_MONSTER) // can't respawn while being eaten
      && ent->s.eType != ET_NPC) {
  ...
  if (ent->s.number < MAX_CLIENTS &&
    ...
    if (g_entities[ent->client->ps.m_iVehicleNum].inuse &&
      ...
      ClientThink(
          ...
          &g_entities[ent->client->ps.m_iVehicleNum].m_pVehicle->m_ucmd);
...
void G_CheckClientTimeouts(gentity_t *ent) {
  ...
  // longer than the timeout to spectator then force this client into spectator
  // mode
...
void ClientThink(int clientNum, usercmd_t *ucmd) {
  ...
  /* 	This was moved to clientthink_real, but since its sort of a risky change
     i left it here for now as a more concrete reference - BSD
          ...
          {//driving a vehicle
                  ...
                  {
                          gentity_t *veh =
     &g_entities[ent->client->ps.m_iVehicleNum];

                          if (veh->m_pVehicle &&
                                  veh->m_pVehicle->m_pPilot == (bgEntity_t
     *)ent) { //only take input from the pilot... veh->client->ps.commandTime =
     ent->client->ps.commandTime; memcpy(&veh->m_pVehicle->m_ucmd,
     &ent->client->pers.cmd, sizeof(usercmd_t)); if (
     veh->m_pVehicle->m_ucmd.buttons & BUTTON_TALK ) { //forced input if "chat
     bubble" is up veh->m_pVehicle->m_ucmd.buttons = BUTTON_TALK;
                                          veh->m_pVehicle->m_ucmd.forwardmove =
     0; veh->m_pVehicle->m_ucmd.rightmove = 0; veh->m_pVehicle->m_ucmd.upmove =
     0;
                                  }
                          }
                  }
  ...
  */
  ...
  // vehicles are clients and when running synchronous they still need to think
  // here so special case them.
     ...
     i left it here for now as a more concrete reference - BSD
          ...
          {//driving a vehicle
                  ...
                  if (g_entities[ent->client->ps.m_iVehicleNum].inuse &&
                          g_entities[ent->client->ps.m_iVehicleNum].client)
                  {
                          ClientThink(ent->client->ps.m_iVehicleNum,
     &g_entities[ent->client->ps.m_iVehicleNum].m_pVehicle->m_ucmd);
                  }
                  else
...
void G_RunClient(gentity_t *ent) {
  ...
  if (!(ent->r.svFlags & SVF_BOT) && g_forceClientUpdateRate.integer &&
    ...
    ent->client->pers.cmd.forwardmove = ent->client->pers.cmd.rightmove =
  ...
  if (ent->client->sess.spectatorState == SPECTATOR_FOLLOW) {
    ...
    if (clientNum >= 0) {
      ...
      if (cl->pers.connected == CON_CONNECTED &&
        ...
        // flags = (cl->mGameFlags & ~(PSG_VOTED | PSG_TEAMVOTED)) |
...
void ClientEndFrame(gentity_t *ent) {
  ...
  // Wacky Mod: Unlimited Regeneration
  if (ent->client->sess.isWackyAdmin) {
    int i;
    ent->client->ps.fd.forcePower = 100;
    if (ent->client->ps.stats[STAT_ARMOR] < 999) {
    ...
    // Refill all ammo
    for (i = 0; i < 16; i++) { // AMMO_MAX is usually not available here easily
                               // without checking, 16 is safe
