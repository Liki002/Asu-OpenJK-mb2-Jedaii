...
#define START_TIME_LINK_ENTS                                                   \
  FRAMETIME * 1 // time-delay after map start at which all ents have been
                // spawned, so can link them
#define START_TIME_FIND_LINKS                                                  \
  FRAMETIME *                                                                  \
      2 // time-delay after map start at which you can find linked entities
#define START_TIME_MOVERS_SPAWNED                                              \
  FRAMETIME *                                                                  \
      2 // time-delay after map start at which all movers should be spawned
#define START_TIME_REMOVE_ENTS                                                 \
  FRAMETIME * 3 // time-delay after map start to remove temporary ents
#define START_TIME_NAV_CALC                                                    \
  FRAMETIME *                                                                  \
      4 // time-delay after map start to connect waypoints and calc routes
#define START_TIME_FIND_WAYPOINT                                               \
  FRAMETIME * 5 // time-delay after map start after which it's okay to try to
                // find your best waypoint
...
struct gentity_s {
  ...
  // rww - these are shared icarus things. They must be in this order as well in
  // relation to the entityshared structure.
  int taskID[NUM_TIDS];
  parms_t *parms;
  ...
  // rww - targetname and classname are now shared as well. ICARUS needs access
  // to them.
  char *targetname;
  char *classname; // set in QuakeEd

  // rww - and yet more things to share. This is because the nav code is in the
  // exe because it's all C++.
  ...
  int noWaypointTime;    // Debouncer - so don't keep checking every waypoint in
                      // existance every frame that you can't find one
  int combatPoint;
  ...
  int cantHitEnemyCounter; // HACK - Makes them look for another enemy on the
                           // same team if the one they're after can't be hit
  ...
  char *target;
  char *target2;
  char *target3; // For multiple targets, not used for firing/triggering/using,
                 // though, only for path branches
  char *target4; // For multiple targets, not used for firing/triggering/using,
                 // though, only for path branches
...
typedef struct clientSession_s {
  ...
  int selectedFP; // check against this, if doesn't match value in playerstate
                  // then update userinfo
  int saberLevel; // similar to above method, but for current saber attack level
  int setForce;   // set to true once player is given the chance to set force
                  // powers
...
typedef struct renderInfo_s {
  ...
  // Fields to apply to entire model set, individual model's equivalents will
  // modify this value
  ...
  vec3_t muzzleDirOld;
  // vec3_t		muzzlePointNext;	// Muzzle point one server frame
  // in the future! vec3_t		muzzleDirNext;
  ...
  vec3_t targetHeadBobAngles; // head bob angles will try to get to
                              // targetHeadBobAngles
  ...
  void *lastG2; // if it doesn't match ent->ghoul2, the bolts are considered
                // invalid.
  int headBolt;
  int handRBolt;
  int handLBolt;
...
struct gclient_s {
  ...
  int g2LastSurfaceHit; // index of surface hit during the most recent ghoul2
                        // collision performed on this client.
  int g2LastSurfaceTime; // time when the surface index was set (to make sure
                         // it's up to date)
  ...
  int saberStoredIndex; // stores saberEntityNum from playerstate for when it's
                        // set to 0 (indicating saber was knocked out of the
                        // air)

  int saberKnockedTime; // if saber gets knocked away, can't pull it back until
                        // this value is < level.time

  vec3_t olderSaberBase; // Set before lastSaberBase_Always, to whatever
                         // lastSaberBase_Always was previously
  ...
  int lastSaberStorageTime;    // server time that the above two values were
                            // updated (for making sure they aren't out of date)
  ...
  int forcePowerSoundDebounce; // if > level.time, don't do certain sound events
                               // again (drain sound, absorb sound, etc)
  ...
  vec3_t hiddenDir; // Normalized direction in which NPCs can't see you (you are
                    // hidden)
  ...
  // keep track of last person kicked and the time so we don't hit multiple
  // times per kick
  int jediKickIndex;
  int jediKickTime;
...
typedef struct alertEvent_s {
  ...
  int ID; // unique... if get a ridiculous number, this will repeat, but should
          // not be a problem as it's just comparing it to your lastAlertID
...
typedef struct level_locals_s {
  ...
  // Interest points- squadmates automatically look at these if standing around
  // and close to them
  interestPoint_t interestPoints[MAX_INTEREST_POINTS];
  int numInterestPoints;

  // Combat points- NPCs in bState BS_COMBAT_POINT will find their closest empty
  // combat_point
...
qboolean G_SpawnBoolean(const char *key, const char *defaultString,
                        qboolean *out);
...
int G_RadiusList(vec3_t origin, float radius, gentity_t *ignore,
                 qboolean takeDamage, gentity_t *ent_list[MAX_GENTITIES]);
...
void G_SetAnim(gentity_t *ent, usercmd_t *ucmd, int setAnimParts, int anim,
               int setAnimFlags, int blendTime);
...
gentity_t *G_ScreenShake(vec3_t org, gentity_t *target, float intensity,
                         int duration, qboolean global);
...
qboolean G_RadiusDamage(vec3_t origin, gentity_t *attacker, float damage,
                        float radius, gentity_t *ignore, gentity_t *missile,
                        int mod);
...
#define DAMAGE_NO_TEAM_PROTECTION                                              \
  0x00000010 // armor, shields, invulnerability, and godmode have no effect
// JK2 flags
#define DAMAGE_EXTRA_KNOCKBACK 0x00000040 // add extra knockback to this damage
#define DAMAGE_DEATH_KNOCKBACK                                                 \
  0x00000080 // only does knockback on death of target
#define DAMAGE_IGNORE_TEAM                                                     \
  0x00000100 // damage is always done, regardless of teams
#define DAMAGE_NO_DAMAGE                                                       \
  0x00000200 // do no actual damage but react as if damage was taken
#define DAMAGE_HALF_ABSORB 0x00000400 // half shields, half health
#define DAMAGE_HALF_ARMOR_REDUCTION                                            \
  0x00000800 // This damage doesn't whittle down armor as efficiently.
#define DAMAGE_HEAVY_WEAP_CLASS 0x00001000 // Heavy damage
#define DAMAGE_NO_HIT_LOC 0x00002000       // No hit location
#define DAMAGE_NO_SELF_PROTECTION                                              \
  0x00004000                           // Dont apply half damage to self attacks
#define DAMAGE_NO_DISMEMBER 0x00008000 // Dont do dismemberment
#define DAMAGE_SABER_KNOCKBACK1                                                \
  0x00010000 // Check the attacker's first saber for a knockbackScale
#define DAMAGE_SABER_KNOCKBACK2                                                \
  0x00020000 // Check the attacker's second saber for a knockbackScale
#define DAMAGE_SABER_KNOCKBACK1_B2                                             \
  0x00040000 // Check the attacker's first saber for a knockbackScale2
#define DAMAGE_SABER_KNOCKBACK2_B2                                             \
  0x00080000 // Check the attacker's second saber for a knockbackScale2
//
// g_exphysics.c
//
void G_RunExPhys(gentity_t *ent, float gravity, float mass, float bounce,
                 qboolean autoKill, int *g2Bolts, int numG2Bolts);
...
reference_tag_t *TAG_Add(const char *name, const char *owner, vec3_t origin,
                         vec3_t angles, int radius, int flags);
...
void WP_FireGenericBlasterMissile(gentity_t *ent, vec3_t start, vec3_t dir,
                                  qboolean altFire, int damage, int velocity,
                                  int mod);
qboolean LogAccuracyHit(gentity_t *target, gentity_t *attacker);
void CalcMuzzlePoint(gentity_t *ent, const vec3_t inForward,
                     const vec3_t inRight, const vec3_t inUp,
                     vec3_t muzzlePoint);
...
// NPC_senses.cpp
//
extern void AddSightEvent(gentity_t *owner, vec3_t position, float radius,
                          alertEventLevel_e alertLevel,
                          float addLight); // addLight = 0.0f
extern void AddSoundEvent(gentity_t *owner, vec3_t position, float radius,
                          alertEventLevel_e alertLevel,
                          qboolean needLOS); // needLOS = qfalse
extern qboolean G_CheckForDanger(gentity_t *self, int alertEvent);
extern int
G_CheckAlertEvents(gentity_t *self, qboolean checkSight, qboolean checkSound,
                   float maxSeeDist, float maxHearDist, int ignoreAlert,
                   qboolean mustHaveOwner,
                   int minAlertLevel); // ignoreAlert = -1, mustHaveOwner =
                                       // qfalse, minAlertLevel = AEL_MINOR
extern qboolean G_CheckForDanger(gentity_t *self, int alertEvent);
extern qboolean G_ClearLOS(gentity_t *self, const vec3_t start,
                           const vec3_t end);
extern qboolean G_ClearLOS2(gentity_t *self, gentity_t *ent, const vec3_t end);
extern qboolean G_ClearLOS3(gentity_t *self, const vec3_t start,
                            gentity_t *ent);
...
int WP_SaberCanBlock(gentity_t *self, vec3_t point, int dflags, int mod,
                     qboolean projectile, int attackStr);
void WP_SaberInitBladeData(gentity_t *ent);
void WP_InitForcePowers(gentity_t *ent);
void WP_SpawnInitForcePowers(gentity_t *ent);
void WP_ForcePowersUpdate(gentity_t *self, usercmd_t *ucmd);
int ForcePowerUsableOn(gentity_t *attacker, gentity_t *other,
...
qboolean Jedi_DodgeEvasion(gentity_t *self, gentity_t *shooter, trace_t *tr,
                           int hitLoc);
...
float NPC_GetHFOVPercentage(vec3_t spot, vec3_t from, vec3_t facing,
                            float hFOV);
float NPC_GetVFOVPercentage(vec3_t spot, vec3_t from, vec3_t facing,
                            float vFOV);
