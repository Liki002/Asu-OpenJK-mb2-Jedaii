...
qboolean BG_SaberStanceAnim(int anim) {
  switch (anim) {
  case BOTH_STAND1: // not really a saberstance anim, actually... "saber off"
                    // stance
...
int BG_BrokenParryForAttack(int move) {
  ...
  // FIXME: need to know which side of the saber was hit!  For now, we presume
  // the saber gets knocked away from the center
...
int BG_BrokenParryForParry(int move) {
  ...
  switch (move) {
  case LS_PARRY_UP:
    // Hmm... since we don't know what dir the hit came from, randomly pick
    // knock down or knock back
    if (Q_irand(0, 1)) {
      return LS_H1_B_;
    } else {
      return LS_H1_T_;
...
qboolean BG_InDeathAnim(
    int anim) { // Purposely does not cover stumbledeath and falldeath...
  switch (anim) {
  ...
  case BOTH_DEATH14_SITUP:  // # Tavion sitting up after having been thrown (cin
                            // #23)
  case BOTH_DEATH15:        // #
  case BOTH_DEATH16:        // #
  case BOTH_DEATH17:        // #
  ...
  case BOTH_FALLDEATH1:    // # Fall forward off a high cliff and splat death -
                           // start
  case BOTH_FALLDEATH1INAIR: // # Fall forward off a high cliff and splat death
                             // - loop
  case BOTH_FALLDEATH1LAND: // # Fall forward off a high cliff and splat death -
                            // hit bottom
  // # #sep case BOTH_ DEAD POSES # Should be last frame of corresponding
  // previous anims
...
qboolean BG_InSaberLock(int anim) {
  ...
  case BOTH_LK_S_S_S_L_1:   // lock if I'm using single vs. a single and I
                            // initiated
  case BOTH_LK_S_S_T_L_1:   // lock if I'm using single vs. a single and I
                            // initiated
  ...
  case BOTH_LK_ST_ST_S_L_1: // lock if I'm using staff vs. a staff and I
                            // initiated
  case BOTH_LK_ST_ST_T_L_1: // lock if I'm using staff vs. a staff and I
                            // initiated
...
qboolean BG_InKnockDownOnGround(playerState_t *ps) {
  ...
  case BOTH_RELEASED:
    // if ( PM_AnimLength(
    // g_entities[ps->clientNum].client->clientInfo.animFileIndex,
    // (animNumber_t)ps->legsAnim ) - ps->legsAnimTimer > 300 )
  ...
  case BOTH_FORCE_GETUP_B6:
    if (BG_AnimLength(0, (animNumber_t)ps->legsAnim) - ps->legsTimer <
        500) { // at beginning of getup anim
      return qtrue;
    }
    break;
  case BOTH_GETUP_BROLL_B:
...
qboolean BG_SaberInReturn(int move) { return PM_SaberInReturn(move); }
...
qboolean PM_CanRollFromSoulCal(playerState_t *ps) {
  if (ps->legsAnim == BOTH_A7_SOULCAL && ps->legsTimer < 700 &&
      ps->legsTimer > 250) {
    return qtrue;
  }
  return qfalse;
...
animation_t
    bgHumanoidAnimations[MAX_TOTALANIMATIONS]; // humanoid animations are the
                                               // only ones that are statically
                                               // allocated.

bgLoadedAnim_t bgAllAnims[MAX_ANIM_FILES];
int bgNumAllAnims = 2; // start off at 2, because 0 will always be assigned to
                       // humanoid, and 1 will always be rockettrooper

// ALWAYS call on game/cgame init
void BG_InitAnimsets(void) {
  memset(&bgAllAnims, 0, sizeof(bgAllAnims));
  BGPAFtextLoaded =
      qfalse; // VVFIXME - The PC doesn't seem to need this, but why?
}
...
animation_t *BG_AnimsetAlloc(void) {
  ...
  bgAllAnims[bgNumAllAnims].anims =
      (animation_t *)BG_Alloc(sizeof(animation_t) * MAX_TOTALANIMATIONS);
...
#ifdef _CGAME // none of this is actually needed serverside. Could just be moved
              // to cgame code but it's here since it used to tie in a lot with
              // the anim loading stuff.
stringID_table_t animEventTypeTable[MAX_ANIM_EVENTS + 1] = {
    ENUM2STRING(AEV_SOUND), // # animID AEV_SOUND framenum soundpath randomlow
                            // randomhi chancetoplay
    ...
    ENUM2STRING(AEV_SOUNDCHAN),   // # animID AEV_SOUNDCHAN framenum CHANNEL
                                  // soundpath randomlow randomhi chancetoplay
    ENUM2STRING(AEV_SABER_SWING), // # animID AEV_SABER_SWING framenum CHANNEL
                                  // randomlow randomhi chancetoplay
    ENUM2STRING(AEV_SABER_SPIN),  // # animID AEV_SABER_SPIN framenum CHANNEL
                                  // chancetoplay
    // must be terminated
    {NULL, -1}};
...
int CheckAnimFrameForEventType(animevent_t *animEvents, int keyFrame,
                               animEventType_t eventType) {
  int i;

  for (i = 0; i < MAX_ANIM_EVENTS; i++) {
    if (animEvents[i].keyFrame ==
        keyFrame) { // there is an animevent on this frame already
      if (animEvents[i].eventType == eventType) { // and it is of the same type
        return i;
      }
    }
  }
...
void ParseAnimationEvtBlock(const char *aeb_filename, animevent_t *animEvents,
                            animation_t *animations, int *i,
                            const char **text_p) {
  const char *token;
  int num, n, animNum, keyFrame, lowestVal, highestVal, curAnimEvent,
      lastAnimEvent = 0;
  animEventType_t eventType;
  ...
  while (1) {
    if (lastAnimEvent >= MAX_ANIM_EVENTS) {
      Com_Error(ERR_DROP,
                "ParseAnimationEvtBlock: number events in animEvent file %s > "
                "MAX_ANIM_EVENTS(%i)",
    ...
    //	just need offsets.
    // This way when animation numbers change, this table won't have to be
    // updated, 	at least not much.
    animNum = GetIDForString(animTable, token);
    if (animNum == -1) { // Unrecognized ANIM ENUM name, or we're skipping this
                         // line, keep going till you get a good one
      Com_Printf(S_COLOR_YELLOW
                 "WARNING: Unknown token %s in animEvent file %s\n",
                 token, aeb_filename);
      while (token[0]) {
        token = COM_ParseExt(
            text_p, qfalse); // returns empty string when next token is EOL
      }
      continue;
    }

    if (animations[animNum].numFrames == 0) { // we don't use this anim
      Com_Printf(S_COLOR_YELLOW
                 "WARNING: %s animevents.cfg: anim %s not used by this model\n",
                 aeb_filename, token);
      // skip this entry
    ...
    if (eventType == AEV_NONE ||
        eventType == (animEventType_t)-1) { // Unrecognized ANIM EVENT TYOE, or
                                            // we're skipping this line, keep
                                            // going till you get a good one
      // Com_Printf(S_COLOR_YELLOW"WARNING: Unknown token %s in animEvent file
      // %s\n", token, aeb_filename );
      continue;
    }
    ...
    // see if this frame already has an event of this type on it, if so,
    // overwrite it
    curAnimEvent = CheckAnimFrameForEventType(animEvents, keyFrame, eventType);
    if (curAnimEvent == -1) { // this anim frame doesn't already have an event
                              // of this type on it
      curAnimEvent = lastAnimEvent;
    }

    // now that we know which event index we're going to plug the data into,
    // start doing it
    ...
    case AEV_SOUNDCHAN: // # animID AEV_SOUNDCHAN framenum CHANNEL soundpath
                        // randomlow randomhi chancetoplay
      token = COM_Parse(text_p);
      if (!token)
        break;
      ...
      if (!Q_stricmp(token, "CHAN_VOICE_ATTEN"))
        ...
        animEvents[curAnimEvent].eventData[AED_SOUNDCHANNEL] =
            CHAN_VOICE_GLOBAL;
    ...
    case AEV_SOUND: // # animID AEV_SOUND framenum soundpath randomlow randomhi
                    // chancetoplay
      // get soundstring
      token = COM_Parse(text_p);
      if (!token) {
      ...
      // NOTE: If we can be assured sequential handles, we can store sound
      // indices 		instead of strings, unfortunately, if these
      // sounds were
      // previously 		registered, we cannot be guaranteed sequential
      // indices.  Thus an array
      if (lowestVal && highestVal) {
        ...
        for (n = lowestVal, num = AED_SOUNDINDEX_START;
             n <= highestVal && num <= AED_SOUNDINDEX_END; n++, num++) {
          if (stringData[0] == '*') { // FIXME? Would be nice to make custom
                                      // sounds work with animEvents.
            animEvents[curAnimEvent].eventData[num] = 0;
          } else {
            animEvents[curAnimEvent].eventData[num] =
                trap->S_RegisterSound(va(stringData, n));
          }
        }
        ...
        if (stringData[0] == '*') { // FIXME? Would be nice to make custom
          ...
          animEvents[curAnimEvent].eventData[AED_SOUNDINDEX_START] =
              trap->S_RegisterSound(stringData);
        }
#ifndef FINAL_BUILD
        if (!animEvents[curAnimEvent].eventData[AED_SOUNDINDEX_START] &&
            stringData[0] != '*') { // couldn't register it - file not found
          Com_Printf(S_COLOR_RED "ParseAnimationSndBlock: sound %s does not "
                                 "exist (animevents.cfg %s)!\n",
      ...
      // last part - cheat and check and see if it's a special overridable saber
      // sound we know of...
      if (!Q_stricmpn("sound/weapons/saber/saberhup", stringData,
                      28)) { // a saber swing
        animEvents[curAnimEvent].eventType = AEV_SABER_SWING;
        animEvents[curAnimEvent].eventData[AED_SABER_SWING_SABERNUM] =
            0; // since we don't know which one they meant if we're hacking
               // this, always use first saber
        animEvents[curAnimEvent].eventData[AED_SABER_SWING_PROBABILITY] =
            animEvents[curAnimEvent].eventData[AED_SOUND_PROBABILITY];
        if (lowestVal < 4) { // fast swing
          animEvents[curAnimEvent].eventData[AED_SABER_SWING_TYPE] =
              0;                    // SWING_FAST;
        } else if (lowestVal < 7) { // medium swing
          animEvents[curAnimEvent].eventData[AED_SABER_SWING_TYPE] =
              1; // SWING_MEDIUM;
        } else { // strong swing
          animEvents[curAnimEvent].eventData[AED_SABER_SWING_TYPE] =
      ...
      } else if (!Q_stricmpn("sound/weapons/saber/saberspin", stringData,
                             29)) { // a saber spin
        animEvents[curAnimEvent].eventType = AEV_SABER_SPIN;
        animEvents[curAnimEvent].eventData[AED_SABER_SPIN_SABERNUM] =
            0; // since we don't know which one they meant if we're hacking
               // this, always use first saber
        animEvents[curAnimEvent].eventData[AED_SABER_SPIN_PROBABILITY] =
            animEvents[curAnimEvent].eventData[AED_SOUND_PROBABILITY];
        if (stringData[29] == 'o') { // saberspinoff
          animEvents[curAnimEvent].eventData[AED_SABER_SPIN_TYPE] = 0;
        } else if (stringData[29] == '1') { // saberspin1
    ...
    case AEV_FOOTSTEP: // # animID AEV_FOOTSTEP framenum footstepType
      ...
      animEvents[curAnimEvent].eventData[AED_FOOTSTEP_TYPE] =
          GetIDForString(footstepTypeTable, token);
      // get probability
      token = COM_Parse(text_p);
      if (!token) { // WARNING!  BAD TABLE!
        break;
      }
      animEvents[curAnimEvent].eventData[AED_FOOTSTEP_PROBABILITY] =
          atoi(token);
      break;
    case AEV_EFFECT: // # animID AEV_EFFECT framenum effectpath boltName
      ...
      animEvents[curAnimEvent].eventData[AED_EFFECTINDEX] =
          trap->FX_RegisterEffect(token);
      // get bolt index
      token = COM_Parse(text_p);
      if (!token) {
        break;
      }
      if (Q_stricmp("none", token) != 0 &&
          Q_stricmp("NULL", token) !=
              0) { // actually are specifying a bolt to use
        if (!animEvents[curAnimEvent].stringData) { // eh, whatever. no dynamic
                                                    // stuff, so this will do.
      ...
      // as below: animEvent->eventData[AED_BOLTINDEX] = trap->G2API_AddBolt(
      // &cent->gent->ghoul2[cent->gent->playerModel], animEvent->stringData );
      // get probability
      token = COM_Parse(text_p);
      if (!token) { // WARNING!  BAD TABLE!
        break;
...
int BG_ParseAnimationEvtFile(const char *as_filename, int animFileIndex,
                             int eventFileIndex) {
  const char *text_p;
  int len;
  const char *token;
  ...
  if (bg_animParseIncluding <=
      0) { // if we should be parsing an included file, skip this part
    if (bgAllEvents[forcedIndex].eventsParsed) { // already cached this one
      return forcedIndex;
    }
  }
  ...
  if (bg_animParseIncluding <=
    ...
    while (i < bgNumAnimEvents && forcedIndex != 0) {
      if (!Q_stricmp(
              as_filename,
              bgAllEvents[i].filename)) { // looks like we have it already.
        return i;
      }
      i++;
    }
  ...
  if (bg_animParseIncluding <= 0) { // should already be done if we're including
    ...
    for (i = 0; i < MAX_ANIM_EVENTS; i++) {
      ...
      // we allow storage of one string, temporarily (in case we have to look up
      // an index later, then make sure to set stringData to NULL so we only do
      // the look-up once)
      torsoAnimEvents[i].stringData = NULL;
      legsAnimEvents[i].stringData = NULL;
      // Unique IDs, can be soundIndex of sound file to play OR effect index or
      // footstep type, etc.
  ...
  while (1) {
    ...
    { // NOTE: you REALLY should NOT do this after the main block of UPPERSOUNDS
      // and LOWERSOUNDS
    ...
    {
      ParseAnimationEvtBlock(as_filename, torsoAnimEvents, animations, &upper_i,
                             &text_p);
    }

    else if (!Q_stricmp(token, "LOWEREVENTS")) // A batch of lower sounds
    {
      ParseAnimationEvtBlock(as_filename, legsAnimEvents, animations, &lower_i,
                             &text_p);
    }
  }
...
fin:
  // Mark this anim set so that we know we tried to load he sounds, don't care
  // if the load failed
  if (bg_animParseIncluding <=
      0) { // if we should be parsing an included file, skip this part
...
======================
*/
int BG_ParseAnimationFile(const char *filename, animation_t *animset,
                          qboolean isHumanoid) {
  char *text_p;
  int len;
  int i;
  ...
  if (!isHumanoid) {
    i = 0;
    while (i < bgNumAllAnims) { // see if it's been loaded already
    ...
    // Looks like it has not yet been loaded. Allocate space for the anim set if
    // we need to, and continue along.
    if (!animset) {
      if (strstr(filename,
                 "players/_humanoid/")) { // then use the static humanoid set.
        animset = bgHumanoidAnimations;
        nextIndex = 0;
      } else if (strstr(filename, "players/rockettrooper/")) { // rockettrooper
                                                               // always index 1
        nextIndex = 1;
        animset = BG_AnimsetAlloc();
        dynAlloc = qtrue; // so we know to free this memory in case we have to
                          // return early. Don't want any leaks.
  ...
  // load the file
  if (!BGPAFtextLoaded ||
      !isHumanoid) { // rww - We are always using the same animation config now.
    ...
    if ((len <= 0) || (len >= sizeof(BGPAFtext) - 1)) {
      ...
      if (len > 0) {
        Com_Error(ERR_DROP,
                  "%s exceeds the allowed game-side animation buffer!",
                  filename);
      }
      return -1;
    }
...
// #ifndef FINAL_BUILD
      ...
      while (token[0]) {
        token =
            COM_ParseExt((const char **)&text_p,
                         qfalse); // returns empty string when next token is EOL
      }
#endif
      continue;
    }
...
/*
        ...
        for(i = 0; i < MAX_ANIMATIONS; i++)
        {
                if (animTable[i].name != NULL)		// This animation
reference exists.
                {
                        if (animset[i].firstFrame <= 0 && animset[i].numFrames
<=0) {	// This is an empty animation reference. Com_Printf("***ANIMTABLE
reference #%d (%s) is empty!\n", i, animTable[i].name);
                        }
                }
        }
#endif // _DEBUG
*/
  ...
  if (isHumanoid) {
    ...
    if (nextIndex > 1) { // don't bother increasing the number if this ended up
                         // as a humanoid/rockettrooper load.
      bgNumAllAnims++;
    } else {
      BGPAFtextLoaded = qtrue;
      usedIndex = nextIndex;
  ...
  { //just loaded humanoid skel - we always want the rockettrooper to be after
it, in slot 1 #ifdef _DEBUG
          assert(BG_ParseAnimationFile("models/players/rockettrooper/animation.cfg",
NULL, qfalse) == 1); #else
          BG_ParseAnimationFile("models/players/rockettrooper/animation.cfg",
NULL, qfalse); #endif
  }
  */
...
#ifdef _GAME
  else if (g_entities[ps->clientNum].s.legsAnim ==
           anim) { // toggled anim to one anim then back to the one we were at
                   // previously in
...
void PM_ForceLegsAnim(int anim) {
  if (BG_InSpecialJump(pm->ps->legsAnim) && pm->ps->legsTimer > 0 &&
      !BG_InSpecialJump(anim)) {
    return;
  }

  if (BG_InRoll(pm->ps, pm->ps->legsAnim) && pm->ps->legsTimer > 0 &&
...
#ifdef _GAME
  else if (g_entities[ps->clientNum].s.torsoAnim ==
           anim) { // toggled anim to one anim then back to the one we were at
                   // previously in
    // one frame, indicating that anim should be restarted.
    BG_FlipPart(ps, SETANIM_TORSO);
...
void PM_StartTorsoAnim(int anim) { BG_StartTorsoAnim(pm->ps, anim); }
...
void BG_SetLegsAnimTimer(playerState_t *ps, int time) {
  ...
  if (ps->legsTimer < 0 &&
      time != -1) { // Cap timer to 0 if was counting down, but let it be -1 if
                    // that was intentional.  NOTENOTE Yeah this seems dumb, but
                    // it mirrors SP.
    ps->legsTimer = 0;
  }
}

void PM_SetLegsAnimTimer(int time) { BG_SetLegsAnimTimer(pm->ps, time); }
...
void PM_SetTorsoAnimTimer(int time) { BG_SetTorsoAnimTimer(pm->ps, time); }

void BG_SaberStartTransAnim(int clientNum, int saberAnimLevel, int weapon,
                            int anim, float *animSpeed, int broken) {
...
void BG_SetAnimFinal(playerState_t *ps, animation_t *animations,
  ...
  BG_SaberStartTransAnim(ps->clientNum, ps->fd.saberAnimLevel, ps->weapon, anim,
                         &editAnimSpeed, ps->brokenLimbs);
  ...
  if (setAnimParts & SETANIM_TORSO) {
    ...
    if (setAnimFlags & SETANIM_FLAG_HOLD) {
      if (setAnimFlags &
          SETANIM_FLAG_HOLDLESS) { // Make sure to only wait in full 1/20 sec
                                   // server frame intervals.
        int dur;
        int speedDif;

        dur = (animations[anim].numFrames - 1) *
              fabs((float)(animations[anim].frameLerp));
        ...
        ps->torsoTimer = ((animations[anim].numFrames) *
                          fabs((float)(animations[anim].frameLerp)));
      }
  ...
  if (setAnimParts & SETANIM_LEGS) {
    ...
    if (setAnimFlags & SETANIM_FLAG_HOLD) {
      if (setAnimFlags &
        ...
        ps->legsTimer = ((animations[anim].numFrames) *
                         fabs((float)(animations[anim].frameLerp)));
      }

      if (PM_RunningAnim(anim) ||
          PM_WalkingAnim(
              anim)) // these guys are ok, they don't actually reference pm
      {
        if (ps->fd.forcePowersActive & (1 << FP_RAGE)) {
          ps->legsTimer /= 1.3;
...
int BG_PickAnim(int animIndex, int minAnim, int maxAnim) {
  ...
  if (count == 1000) { // guess we just don't have a death anim then.
    return -1;
  }
...
// or I will cry. -rww
void BG_SetAnim(playerState_t *ps, animation_t *animations, int setAnimParts,
                int anim, int setAnimFlags) {
  if (!animations) {
    animations = bgAllAnims[0].anims;
  }

  if (animations[anim].firstFrame == 0 && animations[anim].numFrames == 0) {
    if (anim == BOTH_RUNBACK1 || anim == BOTH_WALKBACK1 ||
        anim == BOTH_RUN1) { // hack for droids
      anim = BOTH_WALK2;
    }

    if (animations[anim].firstFrame == 0 &&
        animations[anim].numFrames == 0) { // still? Just return then I guess.
      return;
    }
  }
  ...
  */
  // Don't know why I put this here originally but it's messing stuff up now and
  // it isn't needed.
