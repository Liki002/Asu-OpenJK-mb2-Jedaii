...
char *GVM_ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
  if (gvm->isLegacy)
    return (char *)VM_Call(gvm, GAME_CLIENT_CONNECT, clientNum, firstTime,
                           isBot);
  VMSwap v(gvm);
...
void GVM_ClientThink(int clientNum, usercmd_t *ucmd) {
  if (gvm->isLegacy) {
    VM_Call(gvm, GAME_CLIENT_THINK, clientNum,
            reinterpret_cast<intptr_t>(ucmd));
    return;
  }
  VMSwap v(gvm);
...
void GVM_ROFF_NotetrackCallback(int entID, const char *notetrack) {
  if (gvm->isLegacy) {
    VM_Call(gvm, GAME_ROFF_NOTETRACK_CALLBACK, entID,
            reinterpret_cast<intptr_t>(notetrack));
    return;
  }
  VMSwap v(gvm);
...
qboolean GVM_NAV_ClearPathToPoint(int entID, vec3_t pmins, vec3_t pmaxs,
                                  vec3_t point, int clipmask, int okToHitEnt) {
  if (gvm->isLegacy)
    return (qboolean)VM_Call(
        gvm, GAME_NAV_CLEARPATHTOPOINT, entID,
        reinterpret_cast<intptr_t>(pmins), reinterpret_cast<intptr_t>(pmaxs),
        reinterpret_cast<intptr_t>(point), clipmask, okToHitEnt);
  VMSwap v(gvm);

  return ge->NAV_ClearPathToPoint(entID, pmins, pmaxs, point, clipmask,
                                  okToHitEnt);
}

qboolean GVM_NPC_ClearLOS2(int entID, const vec3_t end) {
  if (gvm->isLegacy)
    return (qboolean)VM_Call(gvm, GAME_NAV_CLEARLOS, entID,
                             reinterpret_cast<intptr_t>(end));
  VMSwap v(gvm);

  return ge->NPC_ClearLOS2(entID, end);
}

int GVM_NAVNEW_ClearPathBetweenPoints(vec3_t start, vec3_t end, vec3_t mins,
                                      vec3_t maxs, int ignore, int clipmask) {
  if (gvm->isLegacy)
    return VM_Call(
        gvm, GAME_NAV_CLEARPATHBETWEENPOINTS, reinterpret_cast<intptr_t>(start),
        reinterpret_cast<intptr_t>(end), reinterpret_cast<intptr_t>(mins),
        reinterpret_cast<intptr_t>(maxs), ignore, clipmask);
  VMSwap v(gvm);

  return ge->NAVNEW_ClearPathBetweenPoints(start, end, mins, maxs, ignore,
                                           clipmask);
}

qboolean GVM_NAV_CheckNodeFailedForEnt(int entID, int nodeNum) {
  if (gvm->isLegacy)
    return (qboolean)VM_Call(gvm, GAME_NAV_CHECKNODEFAILEDFORENT, entID,
                             nodeNum);
  VMSwap v(gvm);
...
int GVM_FS_Write(
    ...
    fileHandle_t f) { // wrapper function to filter things out of log files
  char *msg = (char *)buffer;

  if (com_logChat && com_logChat->integer < 2 && msg) {
    if (!com_logChat->integer &&
        (Q_stristr(msg, "say: ") || Q_stristr(msg, "sayteam: ") ||
         Q_stristr(msg, "tell: "))) // 0 - log nothing
      return len;
    if (com_logChat->integer == 1 &&
        Q_stristr(msg, "tell: ")) // 1 - only log public/team chat
...
static void SV_LocateGameData(sharedEntity_t *gEnts, int numGEntities,
...
static void SV_GameDropClient(int clientNum, const char *reason) {
  ...
  SV_Ranked_HotPotatoDisconnect(clientNum);
...
static void SV_GameSendServerCommand(int clientNum, const char *text) {
  if (text != NULL) {
    ...
    if (strstr(text, "won the round") ||
      ...
      SV_Ranked_ProcessRoundEnd(1); // 1 = RED
    } else if (strstr(text, "SurvivorWinner: Blue")) {
      SV_Ranked_ProcessRoundEnd(2); // 2 = BLUE
    } else if (strstr(text, "Round Active") || strstr(text, "Match Active")) {
      SV_Ranked_ProcessRoundStart();
    }
    // Objective completion: fires when a player completes an objective
    // but NOT when they just score kills (avoids counting raw game score)
    else if (strstr(text, "completed the objective")) {
      Com_Printf("[RANKED] Objective completed event detected\n");
      const char *quote = strchr(text, '"');
      if (quote) {
        SV_SendServerCommand(NULL, "svprint %s", quote);
      }
    }
    // Match Scoreboard Update Hooks - only used for team score tracking now
    else if (Q_stricmpn(text, "scores ", 7) == 0) {
      SV_Ranked_ProcessScoreboard(text);
    }
    else if (Q_stricmpn(text, "ranked_duel_start ", 18) == 0) {
      int p1, p2;
      if (sscanf(text, "ranked_duel_start %d %d", &p1, &p2) == 2) {
        Com_Printf("[RANKED] Duel accepted: %d vs %d\n", p1, p2);
        SV_Ranked_DuelStart(p1, p2);
      }
      return; // Internal command, don't send to clients
    }
    // @@@PLDUELACCEPT broadcast from GVM as a server command (e.g. chat text)
    // Format: "print \"^5Player1 ^7@@@PLDUELACCEPT ^6Player2!\""
    // Strip all color codes first so adjacent ^7@@@PLDUELACCEPT patterns parse cleanly.
    else if (strstr(text, "@@@PLDUELACCEPT")) {
      char stripped[512];
      Q_strncpyz(stripped, text, sizeof(stripped));
      Q_StripColor(stripped);

      const char *marker = strstr(stripped, "@@@PLDUELACCEPT");
      if (marker) {
        char name1[64] = {0};
        char name2[64] = {0};

        // name1: full text from the previous '"' delimiter (or string start)
        // up to the marker. JKA player names can contain spaces, so walking
        // back only to the last space would truncate them and risk resolving
        // a numeric tail (e.g. "zehir 2" -> "2") to the wrong clientNum.
        const char *p = marker - 1;
        while (p > stripped && *p == ' ') p--;
        const char *end1 = p;
        while (p > stripped && *(p-1) != '"' && *(p-1) != '\n') p--;
        int len1 = (int)(end1 - p + 1);
        if (len1 > 0 && len1 < 64) Q_strncpyz(name1, p, len1 + 1);

        // name2: first word after marker, stop at '!' '"' '\n' '\r'
        const char *after = marker + strlen("@@@PLDUELACCEPT");
        while (*after == ' ' || *after == '\t') after++;
        int n2 = 0;
        for (; *after && n2 < 63; after++) {
          if (*after == '!' || *after == '"' || *after == '\n' || *after == '\r') break;
          name2[n2++] = *after;
        }
        name2[n2] = '\0';

        extern int SV_Ranked_FindPlayerByNameOrId(const char *identifier);
        int id1 = name1[0] ? SV_Ranked_FindPlayerByNameOrId(name1) : -1;
        int id2 = name2[0] ? SV_Ranked_FindPlayerByNameOrId(name2) : -1;
        if (id1 >= 0 && id2 >= 0) {
          Com_Printf("[RANKED] @@@PLDUELACCEPT (cmd) resolved: %s(%d) vs %s(%d)\n", name1, id1, name2, id2);
          SV_Ranked_DuelStart(id1, id2);
        } else {
          Com_Printf("[RANKED] @@@PLDUELACCEPT (cmd) FAILED: '%s'=%d '%s'=%d\n", name1, id1, name2, id2);
        }
      }
    }
  }
...
static qboolean SV_EntityContact(const vec3_t mins, const vec3_t maxs,
                                 const sharedEntity_t *gEnt, int capsule) {
...
static void SV_SetBrushModel(sharedEntity_t *ent, const char *name) {
  ...
  if (name[0] == '*') {
    ...
    // rwwNOTE: We don't ever want to set contents -1, it includes
    // CONTENTS_LIGHTSABER. Lots of stuff will explode if there's a brush with
    // CONTENTS_LIGHTSABER that isn't attached to a client owner. ent->contents
    // = -1;		// we don't know exactly what is in the brushes
    h = CM_InlineModel(ent->s.modelindex);
...
static sharedEntity_t *ConvertedEntity(
    sharedEntity_t *ent) { // Return an entity with the memory shifted around to
                           // allow reading/modifying VM memory
  int i = 0;
  ...
  while (i < NUM_BSETS) {
    gLocalModifier.behaviorSet[i] =
        (char *)VM_ArgPtr((intptr_t)ent->behaviorSet[i]);
    i++;
  }
  i = 0;
  gLocalModifier.script_targetname =
      (char *)VM_ArgPtr((intptr_t)ent->script_targetname);
...
static const char *SV_SetActiveSubBSP(int index) {
  if (index >= 0) {
    ...
    sv.mLocalSubBSPEntityParsePoint =
        CM_SubBSPEntityString(sv.mLocalSubBSPIndex);
...
static void SV_PrecisionTimerStart(void **timer) {
  ...
  *timer = newTimer; // assign the pointer within the pointer to point at the
                     // mem addr of our new timer
...
static int SV_PrecisionTimerEnd(void *timer) {
  int r;
  timing_c *theTimer =
      (timing_c *)timer; // this is the pointer we assigned in start, so we can
                         // directly cast it back
  r = theTimer->End();   // get the result
  delete theTimer;       // delete the timer since we're done with it
  return r;              // return the result
}

static void SV_RegisterSharedMemory(char *memory) { sv.mSharedMemory = memory; }
...
qboolean SV_ROFF_Clean(void) { return theROFFSystem.Clean(qfalse); }

void SV_ROFF_UpdateEntities(void) { theROFFSystem.UpdateEntities(qfalse); }

int SV_ROFF_Cache(char *file) { return theROFFSystem.Cache(file, qfalse); }
...
static qboolean SV_ICARUS_RegisterScript(const char *name,
...
static void SV_Nav_Init(void) { navigator.Init(); }

static void SV_Nav_Free(void) { navigator.Free(); }
...
static void SV_Nav_ShowNodes(void) { navigator.ShowNodes(); }

static void SV_Nav_ShowEdges(void) { navigator.ShowEdges(); }

static void SV_Nav_ShowPath(int start, int end) {
  navigator.ShowPath(start, end);
}

static int SV_Nav_GetNearestNode(sharedEntity_t *ent, int lastID, int flags,
                                 int targetID) {
...
static qboolean SV_Nav_RouteBlocked(int startID, int testEdgeID, int endID,
                                    int rejectRank) {
  return navigator.RouteBlocked(startID, testEdgeID, endID, rejectRank);
}

static int SV_Nav_GetBestNodeAltRoute(int startID, int endID, int *pathCost,
                                      int rejectID) {
...
static int SV_Nav_GetBestPathBetweenEnts(sharedEntity_t *ent,
                                         sharedEntity_t *goal, int flags) {
...
static void SV_Nav_CheckBlockedEdges(void) { navigator.CheckBlockedEdges(); }

static void SV_Nav_ClearCheckedNodes(void) { navigator.ClearCheckedNodes(); }
...
static float SV_Characteristic_BFloat(int character, int index, float min,
                                      float max) {
...
static int SV_Characteristic_BInteger(int character, int index, int min,
                                      int max) {
  return botlib_export->ai.Characteristic_BInteger(character, index, min, max);
}

static void SV_Characteristic_String(int character, int index, char *buf,
                                     int size) {
...
static int SV_BotNextConsoleMessage(int chatstate, void *cm) {
  return botlib_export->ai.BotNextConsoleMessage(chatstate,
                                                 (bot_consolemessage_s *)cm);
}
...
static void SV_BotInitialChat(int chatstate, char *type, int mcontext,
                              char *var0, char *var1, char *var2, char *var3,
                              char *var4, char *var5, char *var6, char *var7) {
  botlib_export->ai.BotInitialChat(chatstate, type, mcontext, var0, var1, var2,
                                   var3, var4, var5, var6, var7);
}

static int SV_BotReplyChat(int chatstate, char *message, int mcontext,
                           int vcontext, char *var0, char *var1, char *var2,
                           char *var3, char *var4, char *var5, char *var6,
                           char *var7) {
  return botlib_export->ai.BotReplyChat(chatstate, message, mcontext, vcontext,
...
static void SV_BotMatchVariable(void *match, int variable, char *buf,
                                int size) {
...
static int SV_BotChooseLTGItem(int goalstate, vec3_t origin, int *inventory,
                               int travelflags) {
  return botlib_export->ai.BotChooseLTGItem(goalstate, origin, inventory,
                                            travelflags);
}

static int SV_BotChooseNBGItem(int goalstate, vec3_t origin, int *inventory,
                               int travelflags, void *ltg, float maxtime) {
  return botlib_export->ai.BotChooseNBGItem(
      goalstate, origin, inventory, travelflags, (bot_goal_s *)ltg, maxtime);
}
...
static int SV_BotItemGoalInVisButNotVisible(int viewer, vec3_t eye,
                                            vec3_t viewangles, void *goal) {
  return botlib_export->ai.BotItemGoalInVisButNotVisible(
      viewer, eye, viewangles, (bot_goal_s *)goal);
}

static int SV_BotGetLevelItemGoal(int index, char *classname, void *goal) {
  return botlib_export->ai.BotGetLevelItemGoal(index, classname,
                                               (bot_goal_s *)goal);
}
...
static void SV_BotMoveToGoal(void *result, int movestate, void *goal,
                             int travelflags) {
  botlib_export->ai.BotMoveToGoal((bot_moveresult_s *)result, movestate,
                                  (bot_goal_s *)goal, travelflags);
}

static int SV_BotMoveInDirection(int movestate, vec3_t dir, float speed,
                                 int type) {
...
static int SV_BotMovementViewTarget(int movestate, void *goal, int travelflags,
                                    float lookahead, vec3_t target) {
  return botlib_export->ai.BotMovementViewTarget(
      movestate, (bot_goal_s *)goal, travelflags, lookahead, target);
}
...
static void SV_BotGetWeaponInfo(int weaponstate, int weapon, void *weaponinfo) {
  botlib_export->ai.BotGetWeaponInfo(weaponstate, weapon,
...
static int SV_GeneticParentsAndChildSelection(int numranks, float *ranks,
                                              int *parent1, int *parent2,
                                              int *child) {
  return botlib_export->ai.GeneticParentsAndChildSelection(
      numranks, ranks, parent1, parent2, child);
}

static void SV_BotInterbreedGoalFuzzyLogic(int parent1, int parent2,
                                           int child) {
...
static int SV_BotPredictVisiblePosition(vec3_t origin, int areanum, void *goal,
                                        int travelflags, vec3_t target) {
  return botlib_export->ai.BotPredictVisiblePosition(
      origin, areanum, (bot_goal_s *)goal, travelflags, target);
}
...
static void SV_BotAddAvoidSpot(int movestate, vec3_t origin, float radius,
                               int type) {
  botlib_export->ai.BotAddAvoidSpot(movestate, origin, radius, type);
}

static int SV_BotLibSetup(void) { return botlib_export->BotLibSetup(); }

static int SV_BotLibShutdown(void) { return botlib_export->BotLibShutdown(); }
...
static int SV_AAS_BBoxAreas(vec3_t absmins, vec3_t absmaxs, int *areas,
                            int maxareas) {
...
static void SV_AAS_PresenceTypeBoundingBox(int presencetype, vec3_t mins,
                                           vec3_t maxs) {
  botlib_export->aas.AAS_PresenceTypeBoundingBox(presencetype, mins, maxs);
}

static float SV_AAS_Time(void) { return botlib_export->aas.AAS_Time(); }

static int SV_AAS_PointAreaNum(vec3_t point) {
  return botlib_export->aas.AAS_PointAreaNum(point);
}

static int SV_AAS_TraceAreas(vec3_t start, vec3_t end, int *areas,
...
static int SV_AAS_ValueForBSPEpairKey(int ent, char *key, char *value,
                                      int size) {
...
static int SV_AAS_AreaTravelTimeToGoalArea(int areanum, vec3_t origin,
                                           int goalareanum, int travelflags) {
  return botlib_export->aas.AAS_AreaTravelTimeToGoalArea(
      areanum, origin, goalareanum, travelflags);
}
...
static int SV_AAS_PredictClientMovement(void *move, int entnum, vec3_t origin,
  ...
  return botlib_export->aas.AAS_PredictClientMovement(
      (aas_clientmove_s *)move, entnum, origin, presencetype, onground,
      velocity, cmdmove, cmdframes, maxframes, frametime, stopevent,
      stopareanum, visualize);
}

static int SV_AAS_AlternativeRouteGoals(vec3_t start, int startareanum,
                                        vec3_t goal, int goalareanum,
                                        int travelflags, void *altroutegoals,
                                        int maxaltroutegoals, int type) {
  return botlib_export->aas.AAS_AlternativeRouteGoals(
      start, startareanum, goal, goalareanum, travelflags,
      (aas_altroutegoal_s *)altroutegoals, maxaltroutegoals, type);
}

static int SV_AAS_PredictRoute(void *route, int areanum, vec3_t origin,
                               int goalareanum, int travelflags, int maxareas,
                               int maxtime, int stopevent, int stopcontents,
                               int stoptfl, int stopareanum) {
  return botlib_export->aas.AAS_PredictRoute(
      (aas_predictroute_s *)route, areanum, origin, goalareanum, travelflags,
      maxareas, maxtime, stopevent, stopcontents, stoptfl, stopareanum);
}
...
static void SV_EA_Gesture(int client) { botlib_export->ea.EA_Gesture(client); }

static void SV_EA_Talk(int client) { botlib_export->ea.EA_Talk(client); }

static void SV_EA_Attack(int client) { botlib_export->ea.EA_Attack(client); }
...
static void SV_EA_Use(int client) { botlib_export->ea.EA_Use(client); }

static void SV_EA_Respawn(int client) { botlib_export->ea.EA_Respawn(client); }

static void SV_EA_Crouch(int client) { botlib_export->ea.EA_Crouch(client); }

static void SV_EA_MoveUp(int client) { botlib_export->ea.EA_MoveUp(client); }
...
static void SV_EA_Jump(int client) { botlib_export->ea.EA_Jump(client); }
...
static int SV_CM_RegisterTerrain(const char *config) { return 0; }
...
static void SV_G2API_SetGhoul2ModelIndexes(void *ghoul2, qhandle_t *modelList,
                                           qhandle_t *skinList) {
  if (!ghoul2)
    return;
  re->G2API_SetGhoul2ModelIndexes(*((CGhoul2Info_v *)ghoul2), modelList,
                                  skinList);
}
...
static qboolean SV_G2API_GetBoltMatrix(void *ghoul2, const int modelIndex,
  ...
  if (!ghoul2)
    return qfalse;
  return re->G2API_GetBoltMatrix(*((CGhoul2Info_v *)ghoul2), modelIndex,
                                 boltIndex, matrix, angles, position, frameNum,
                                 modelList, scale);
}

static qboolean SV_G2API_GetBoltMatrix_NoReconstruct(
    void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix,
...
static qboolean SV_G2API_GetBoltMatrix_NoRecNoRot(
    void *ghoul2, const int modelIndex, const int boltIndex, mdxaBone_t *matrix,
    const vec3_t angles, const vec3_t position, const int frameNum,
    qhandle_t *modelList, vec3_t scale) {
  if (!ghoul2)
...
static int SV_G2API_InitGhoul2Model(void **ghoul2Ptr, const char *fileName,
                                    int modelIndex, qhandle_t customSkin,
                                    qhandle_t customShader, int modelFlags,
                                    int lodBias) {
#ifdef _FULL_G2_LEAK_CHECKING
  g_G2AllocServer = 1;
#endif
  return re->G2API_InitGhoul2Model((CGhoul2Info_v **)ghoul2Ptr, fileName,
                                   modelIndex, customSkin, customShader,
                                   modelFlags, lodBias);
}

static qboolean SV_G2API_SetSkin(void *ghoul2, int modelIndex,
                                 qhandle_t customSkin, qhandle_t renderSkin) {
  if (!ghoul2)
    return qfalse;
...
static void SV_G2API_CollisionDetect(CollisionRecord_t *collRecMap,
                                     void *ghoul2, const vec3_t angles,
                                     const vec3_t position, int frameNumber,
                                     int entNum, vec3_t rayStart, vec3_t rayEnd,
                                     vec3_t scale, int traceFlags, int useLod,
                                     float fRadius) {
  if (!ghoul2)
    return;
  re->G2API_CollisionDetect(collRecMap, *((CGhoul2Info_v *)ghoul2), angles,
                            position, frameNumber, entNum, rayStart, rayEnd,
                            scale, G2VertSpaceServer, traceFlags, useLod,
                            fRadius);
}

static void SV_G2API_CollisionDetectCache(
    CollisionRecord_t *collRecMap, void *ghoul2, const vec3_t angles,
    const vec3_t position, int frameNumber, int entNum, vec3_t rayStart,
    vec3_t rayEnd, vec3_t scale, int traceFlags, int useLod, float fRadius) {
  if (!ghoul2)
    return;
  re->G2API_CollisionDetectCache(collRecMap, *((CGhoul2Info_v *)ghoul2), angles,
                                 position, frameNumber, entNum, rayStart,
                                 rayEnd, scale, G2VertSpaceServer, traceFlags,
                                 useLod, fRadius);
}
...
static qboolean SV_G2API_SetBoneAngles(void *ghoul2, int modelIndex,
                                       const char *boneName,
                                       const vec3_t angles, const int flags,
                                       const int up, const int right,
                                       const int forward, qhandle_t *modelList,
                                       int blendTime, int currentTime) {
  if (!ghoul2)
    return qfalse;
  return re->G2API_SetBoneAngles(
      *((CGhoul2Info_v *)ghoul2), modelIndex, boneName, angles, flags,
      (const Eorientations)up, (const Eorientations)right,
      (const Eorientations)forward, modelList, blendTime, currentTime);
}

static qboolean
SV_G2API_SetBoneAnim(void *ghoul2, const int modelIndex, const char *boneName,
                     const int startFrame, const int endFrame, const int flags,
                     const float animSpeed, const int currentTime,
                     const float setFrame, const int blendTime) {
  if (!ghoul2)
    return qfalse;
  return re->G2API_SetBoneAnim(*((CGhoul2Info_v *)ghoul2), modelIndex, boneName,
                               startFrame, endFrame, flags, animSpeed,
                               currentTime, setFrame, blendTime);
}

static qboolean SV_G2API_GetBoneAnim(void *ghoul2, const char *boneName,
                                     const int currentTime, float *currentFrame,
                                     int *startFrame, int *endFrame, int *flags,
                                     float *animSpeed, int *modelList,
                                     const int modelIndex) {
  if (!ghoul2)
    return qfalse;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
  return re->G2API_GetBoneAnim(g2, modelIndex, boneName, currentTime,
                               currentFrame, startFrame, endFrame, flags,
                               animSpeed, modelList);
}
...
static int SV_G2API_CopyGhoul2Instance(void *g2From, void *g2To,
                                       int modelIndex) {
  if (!g2From || !g2To)
    return 0;
  return re->G2API_CopyGhoul2Instance(*((CGhoul2Info_v *)g2From),
                                      *((CGhoul2Info_v *)g2To), modelIndex);
}
...
static void SV_G2API_DuplicateGhoul2Instance(void *g2From, void **g2To) {
  ...
  if (!g2From || !g2To)
    return;
  re->G2API_DuplicateGhoul2Instance(*((CGhoul2Info_v *)g2From),
                                    (CGhoul2Info_v **)g2To);
}
...
static int SV_G2API_AddBolt(void *ghoul2, int modelIndex,
                            const char *boneName) {
  if (!ghoul2)
    return -1;
...
static qboolean SV_G2API_SetRootSurface(void *ghoul2, const int modelIndex,
                                        const char *surfaceName) {
  if (!ghoul2)
    return qfalse;
  return re->G2API_SetRootSurface(*((CGhoul2Info_v *)ghoul2), modelIndex,
                                  surfaceName);
}

static qboolean SV_G2API_SetSurfaceOnOff(void *ghoul2, const char *surfaceName,
                                         const int flags) {
  if (!ghoul2)
    return qfalse;
  return re->G2API_SetSurfaceOnOff(*((CGhoul2Info_v *)ghoul2), surfaceName,
                                   flags);
}
...
static qboolean SV_G2API_DoesBoneExist(void *ghoul2, int modelIndex,
                                       const char *boneName) {
  if (!ghoul2)
    return qfalse;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
...
static int SV_G2API_GetSurfaceRenderStatus(void *ghoul2, const int modelIndex,
                                           const char *surfaceName) {
  if (!ghoul2)
    return -1;
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
...
static void SV_G2API_SetRagDoll(void *ghoul2, sharedRagDollParams_t *params) {
  ...
  rdParams.effectorsToTurnOff =
      (CRagDollParams::ERagEffector)params->effectorsToTurnOff;

  re->G2API_SetRagDoll(*((CGhoul2Info_v *)ghoul2), &rdParams);
}

static void SV_G2API_AnimateG2Models(void *ghoul2, int time,
...
static qboolean SV_G2API_RagPCJConstraint(void *ghoul2, const char *boneName,
                                          vec3_t min, vec3_t max) {
  return re->G2API_RagPCJConstraint(*((CGhoul2Info_v *)ghoul2), boneName, min,
                                    max);
}

static qboolean SV_G2API_RagPCJGradientSpeed(void *ghoul2, const char *boneName,
                                             const float speed) {
  return re->G2API_RagPCJGradientSpeed(*((CGhoul2Info_v *)ghoul2), boneName,
                                       speed);
}

static qboolean SV_G2API_RagEffectorGoal(void *ghoul2, const char *boneName,
                                         vec3_t pos) {
  return re->G2API_RagEffectorGoal(*((CGhoul2Info_v *)ghoul2), boneName, pos);
}

static qboolean SV_G2API_GetRagBonePos(void *ghoul2, const char *boneName,
                                       vec3_t pos, vec3_t entAngles,
                                       vec3_t entPos, vec3_t entScale) {
  return re->G2API_GetRagBonePos(*((CGhoul2Info_v *)ghoul2), boneName, pos,
                                 entAngles, entPos, entScale);
}

static qboolean SV_G2API_RagEffectorKick(void *ghoul2, const char *boneName,
                                         vec3_t velocity) {
  return re->G2API_RagEffectorKick(*((CGhoul2Info_v *)ghoul2), boneName,
                                   velocity);
}
...
static qboolean SV_G2API_SetBoneIKState(void *ghoul2, int time,
                                        const char *boneName, int ikState,
                                        sharedSetBoneIKStateParams_t *params) {
  return re->G2API_SetBoneIKState(*((CGhoul2Info_v *)ghoul2), time, boneName,
                                  ikState, params);
}

static qboolean SV_G2API_IKMove(void *ghoul2, int time,
                                sharedIKMoveParams_t *params) {
  return re->G2API_IKMove(*((CGhoul2Info_v *)ghoul2), time, params);
}

static qboolean SV_G2API_RemoveBone(void *ghoul2, const char *boneName,
                                    int modelIndex) {
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
  return re->G2API_RemoveBone(g2, modelIndex, boneName);
}

static void SV_G2API_AttachInstanceToEntNum(void *ghoul2, int entityNum,
                                            qboolean server) {
  re->G2API_AttachInstanceToEntNum(*((CGhoul2Info_v *)ghoul2), entityNum,
                                   server);
}
...
static void SV_G2API_GetSurfaceName(void *ghoul2, int surfNumber,
                                    int modelIndex, char *fillBuf) {
  CGhoul2Info_v &g2 = *((CGhoul2Info_v *)ghoul2);
...
intptr_t SV_GameSystemCalls(intptr_t *args) {
  switch (args[0]) {
    ...
    // adding a trap to match, and all of these traps must be shared and have
    // cases in sv_game, cl_cgame, and cl_ui. They must also all be in the same
    // order, and start at 100.
  case TRAP_MEMSET:
  ...
  case TRAP_ANGLEVECTORS:
    AngleVectors((const float *)VMA(1), (float *)VMA(2), (float *)VMA(3),
                 (float *)VMA(4));
    return 0;
  ...
  case G_PRINT: {
    const char *gmsg = (const char *)VMA(1);
    // RANKED RANKED: Intercept Kill events from the legacy VM game module
    if (gmsg && Q_stricmpn(gmsg, "Kill: ", 6) == 0) {
      int killerId = -1, victimId = -1, mod = -1;
      if (sscanf(gmsg, "Kill: %d %d %d:", &killerId, &victimId, &mod) == 3) {
        char wep[64] = "Unknown";
        const char *wMatch = strstr(gmsg, " by ");
        if (wMatch) {
          Q_strncpyz(wep, wMatch + 4, sizeof(wep));
          char *nl = strchr(wep, '\n');
          if (nl) *nl = '\0';
          char *cr = strchr(wep, '\r');
          if (cr) *cr = '\0';
        }
        // Extract victim display name between " killed " and " by " for NPC matching
        char vname[64] = "";
        const char *kMatch = strstr(gmsg, " killed ");
        if (kMatch && wMatch && wMatch > kMatch) {
          int len = (int)(wMatch - (kMatch + 8));
          if (len > 0 && len < (int)sizeof(vname)) {
            memcpy(vname, kMatch + 8, len);
            vname[len] = '\0';
          }
        }
        Com_Printf("[RANKED] ParsedKill: killer=%d victim=%d mod=%d wep=%s vname='%s'\n", killerId, victimId, mod, wep, vname);
        SV_Ranked_ProcessKill(killerId, victimId, mod, wep, vname);
        SV_Ranked_HotPotatoHandleKill(killerId, victimId);
      } else {
        Com_Printf("[RANKED] Kill line parse FAILED: %s", gmsg);
      }
    } else if (strstr(gmsg, "red:") && strstr(gmsg, "blue:")) {
      const char *redMatch = strstr(gmsg, "red:");
      const char *blueMatch = strstr(gmsg, "blue:");
      int scoreRed = 0, scoreBlue = 0;
      sscanf(redMatch, "red:%d", &scoreRed);
      sscanf(blueMatch, "blue:%d", &scoreBlue);
      int winner = (scoreRed > scoreBlue) ? 1 : ((scoreBlue > scoreRed) ? 2 : 0);
      Com_Printf("[RANKED] End Game Team Scores Parsed: Red %d, Blue %d (Winner %d)\n", scoreRed, scoreBlue, winner);
      SV_Ranked_ProcessRoundEnd(winner);
    } else if (strstr(gmsg, "Successful SMOD login by")) {
      // Parse: "Successful SMOD login by NAME (adminID: N) (IP: ...)"
      // Extract name and adminID
      const char *byPos = strstr(gmsg, "Successful SMOD login by ");
      if (byPos) {
        byPos += strlen("Successful SMOD login by ");
        char smodName[MAX_STRING_CHARS];
        Q_strncpyz(smodName, byPos, sizeof(smodName));
        // Trim at " (adminID:"
        char *parenPos = strstr(smodName, " (adminID:");
        int adminId = 0;
        if (parenPos) {
          sscanf(parenPos, " (adminID: %d)", &adminId);
          *parenPos = '\0';
        }
        if (adminId > 0 && smodName[0]) {
          // Find player by name and push admin status to their DB account
          int targetClient = SV_Ranked_FindPlayerByNameOrId(smodName);
          if (targetClient >= 0) {
            SV_Ranked_SetAdmin(targetClient, adminId);
          }
        }
      }
    } else if (strstr(gmsg, "@@@PLDUELACCEPT")) {
      // Strip all color codes first so adjacent ^7@@@PLDUELACCEPT patterns parse cleanly.
      char stripped[512];
      Q_strncpyz(stripped, gmsg, sizeof(stripped));
      Q_StripColor(stripped);
      ...
      if (marker) {
        ...
        // name1: full text from the previous '"' delimiter (or string start)
        // up to the marker. Player names may contain spaces — walking back
        // only to the last space would truncate them.
        const char *p = marker - 1;
        while (p > stripped && *p == ' ') p--;
        const char *end1 = p;
        while (p > stripped && *(p-1) != '"' && *(p-1) != '\n') p--;
        ...
        int id2 = name2[0] ? SV_Ranked_FindPlayerByNameOrId(name2) : -1;
        if (id1 >= 0 && id2 >= 0) {
          Com_Printf("[RANKED] @@@PLDUELACCEPT (print) resolved: %s(%d) vs %s(%d)\n", name1, id1, name2, id2);
          SV_Ranked_DuelStart(id1, id2);
        } else {
          Com_Printf("[RANKED] @@@PLDUELACCEPT (print) FAILED: '%s'=%d '%s'=%d\n", name1, id1, name2, id2);
        }
      }
    }
    Com_Printf("%s", gmsg);
  ...
  case G_CVAR_REGISTER:
    Cvar_Register((vmCvar_t *)VMA(1), (const char *)VMA(2),
                  (const char *)VMA(3), args[4]);
    return 0;
  ...
  case G_FS_FOPEN_FILE:
    return FS_FOpenFileByMode((const char *)VMA(1), (int *)VMA(2),
                              (fsMode_t)args[3]);
  ...
  case G_FS_GETFILELIST:
    return FS_GetFileList((const char *)VMA(1), (const char *)VMA(2),
                          (char *)VMA(3), args[4]);

  case G_LOCATE_GAME_DATA:
    SV_LocateGameData((sharedEntity_t *)VMA(1), args[2], args[3],
                      (struct playerState_s *)VMA(4), args[5]);
    return 0;
  ...
  case G_ENTITIES_IN_BOX:
    return SV_AreaEntities((const float *)VMA(1), (const float *)VMA(2),
                           (int *)VMA(3), args[4]);

  case G_ENTITY_CONTACT:
    return SV_EntityContact((const float *)VMA(1), (const float *)VMA(2),
                            (const sharedEntity_t *)VMA(3),
                            /*int capsule*/ qfalse);
  ...
  case G_TRACE:
    SV_Trace((trace_t *)VMA(1), (const float *)VMA(2), (const float *)VMA(3),
             (const float *)VMA(4), (const float *)VMA(5), args[6], args[7],
             /*int capsule*/ qfalse, /*args[8]*/ 0, args[9]);
    return 0;
  case G_G2TRACE:
    SV_Trace((trace_t *)VMA(1), (const float *)VMA(2), (const float *)VMA(3),
             (const float *)VMA(4), (const float *)VMA(5), args[6], args[7],
             /*int capsule*/ qfalse, args[8], args[9]);
    return 0;
  case G_TRACECAPSULE:
    SV_Trace((trace_t *)VMA(1), (const float *)VMA(2), (const float *)VMA(3),
             (const float *)VMA(4), (const float *)VMA(5), args[6], args[7],
             /*int capsule*/ qtrue, args[8], args[9]);
    return 0;
...
case G_BOT_FREE_MEMORY:
  ...
  case G_ICARUS_RUNSCRIPT:
    return ICARUS_RunScript(ConvertedEntity((sharedEntity_t *)VMA(1)),
                            (const char *)VMA(2));
  ...
  case G_NAV_GETNEARESTNODE:
    return navigator.GetNearestNode((sharedEntity_t *)VMA(1), args[2], args[3],
                                    args[4]);
  ...
  case G_NAV_GETBESTNODEALTROUTE:
    return navigator.GetBestNodeAltRoute(args[1], args[2], (int *)VMA(3),
                                         args[4]);
  case G_NAV_GETBESTNODEALT2:
    return navigator.GetBestNodeAltRoute(args[1], args[2], args[3]);
  case G_NAV_GETBESTPATHBETWEENENTS:
    return navigator.GetBestPathBetweenEnts((sharedEntity_t *)VMA(1),
                                            (sharedEntity_t *)VMA(2), args[3]);
  ...
  case BOTLIB_PC_READ_TOKEN:
    return botlib_export->PC_ReadTokenHandle(args[1],
                                             (struct pc_token_s *)VMA(2));
  case BOTLIB_PC_SOURCE_FILE_AND_LINE:
    return botlib_export->PC_SourceFileAndLine(args[1], (char *)VMA(2),
                                               (int *)VMA(3));
  ...
  case BOTLIB_UPDATENTITY:
    return botlib_export->BotLibUpdateEntity(
        args[1], (struct bot_entitystate_s *)VMA(2));
  case BOTLIB_TEST:
    return botlib_export->Test(args[1], (char *)VMA(2), (float *)VMA(3),
                               (float *)VMA(4));
  ...
  case BOTLIB_AAS_BBOX_AREAS:
    return botlib_export->aas.AAS_BBoxAreas((float *)VMA(1), (float *)VMA(2),
                                            (int *)VMA(3), args[4]);
  case BOTLIB_AAS_AREA_INFO:
    return botlib_export->aas.AAS_AreaInfo(args[1],
                                           (struct aas_areainfo_s *)VMA(2));
  case BOTLIB_AAS_ALTERNATIVE_ROUTE_GOAL:
    return botlib_export->aas.AAS_AlternativeRouteGoals(
        (float *)VMA(1), args[2], (float *)VMA(3), args[4], args[5],
        (struct aas_altroutegoal_s *)VMA(6), args[7], args[8]);
  case BOTLIB_AAS_ENTITY_INFO:
    botlib_export->aas.AAS_EntityInfo(args[1],
                                      (struct aas_entityinfo_s *)VMA(2));
    return 0;

  case BOTLIB_AAS_INITIALIZED:
    return botlib_export->aas.AAS_Initialized();
  case BOTLIB_AAS_PRESENCE_TYPE_BOUNDING_BOX:
    botlib_export->aas.AAS_PresenceTypeBoundingBox(args[1], (float *)VMA(2),
                                                   (float *)VMA(3));
    return 0;
  ...
  case BOTLIB_AAS_TRACE_AREAS:
    return botlib_export->aas.AAS_TraceAreas((float *)VMA(1), (float *)VMA(2),
                                             (int *)VMA(3),
                                             (float (*)[3])VMA(4), args[5]);
  ...
  case BOTLIB_AAS_VALUE_FOR_BSP_EPAIR_KEY:
    return botlib_export->aas.AAS_ValueForBSPEpairKey(args[1], (char *)VMA(2),
                                                      (char *)VMA(3), args[4]);
  case BOTLIB_AAS_VECTOR_FOR_BSP_EPAIR_KEY:
    return botlib_export->aas.AAS_VectorForBSPEpairKey(args[1], (char *)VMA(2),
                                                       (float *)VMA(3));
  case BOTLIB_AAS_FLOAT_FOR_BSP_EPAIR_KEY:
    return botlib_export->aas.AAS_FloatForBSPEpairKey(args[1], (char *)VMA(2),
                                                      (float *)VMA(3));
  case BOTLIB_AAS_INT_FOR_BSP_EPAIR_KEY:
    return botlib_export->aas.AAS_IntForBSPEpairKey(args[1], (char *)VMA(2),
                                                    (int *)VMA(3));
  ...
  case BOTLIB_AAS_PREDICT_ROUTE:
    return botlib_export->aas.AAS_PredictRoute(
        (struct aas_predictroute_s *)VMA(1), args[2], (float *)VMA(3), args[4],
  ...
  case BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT:
    return botlib_export->aas.AAS_PredictClientMovement(
        (struct aas_clientmove_s *)VMA(1), args[2], (float *)VMA(3), args[4],
        args[5], (float *)VMA(6), (float *)VMA(7), args[8], args[9], VMF(10),
  ...
  case BOTLIB_EA_GET_INPUT:
    botlib_export->ea.EA_GetInput(args[1], VMF(2),
                                  (struct bot_input_s *)VMA(3));
    return 0;
  ...
  case BOTLIB_AI_CHARACTERISTIC_BFLOAT:
    return FloatAsInt(botlib_export->ai.Characteristic_BFloat(args[1], args[2],
                                                              VMF(3), VMF(4)));
  ...
  case BOTLIB_AI_CHARACTERISTIC_STRING:
    botlib_export->ai.Characteristic_String(args[1], args[2], (char *)VMA(3),
                                            args[4]);
    return 0;
  ...
  case BOTLIB_AI_NEXT_CONSOLE_MESSAGE:
    return botlib_export->ai.BotNextConsoleMessage(
        args[1], (struct bot_consolemessage_s *)VMA(2));
  case BOTLIB_AI_NUM_CONSOLE_MESSAGE:
    return botlib_export->ai.BotNumConsoleMessages(args[1]);
  case BOTLIB_AI_INITIAL_CHAT:
    botlib_export->ai.BotInitialChat(
        args[1], (char *)VMA(2), args[3], (char *)VMA(4), (char *)VMA(5),
        (char *)VMA(6), (char *)VMA(7), (char *)VMA(8), (char *)VMA(9),
        (char *)VMA(10), (char *)VMA(11));
    return 0;
  case BOTLIB_AI_NUM_INITIAL_CHATS:
    return botlib_export->ai.BotNumInitialChats(args[1], (char *)VMA(2));
  case BOTLIB_AI_REPLY_CHAT:
    return botlib_export->ai.BotReplyChat(
        args[1], (char *)VMA(2), args[3], args[4], (char *)VMA(5),
        (char *)VMA(6), (char *)VMA(7), (char *)VMA(8), (char *)VMA(9),
        (char *)VMA(10), (char *)VMA(11), (char *)VMA(12));
  ...
  case BOTLIB_AI_STRING_CONTAINS:
    return botlib_export->ai.StringContains((char *)VMA(1), (char *)VMA(2),
                                            args[3]);
  case BOTLIB_AI_FIND_MATCH:
    return botlib_export->ai.BotFindMatch(
        (char *)VMA(1), (struct bot_match_s *)VMA(2), args[3]);
  case BOTLIB_AI_MATCH_VARIABLE:
    botlib_export->ai.BotMatchVariable((struct bot_match_s *)VMA(1), args[2],
                                       (char *)VMA(3), args[4]);
    return 0;
  ...
  case BOTLIB_AI_LOAD_CHAT_FILE:
    return botlib_export->ai.BotLoadChatFile(args[1], (char *)VMA(2),
                                             (char *)VMA(3));
  ...
  case BOTLIB_AI_GET_TOP_GOAL:
    return botlib_export->ai.BotGetTopGoal(args[1],
                                           (struct bot_goal_s *)VMA(2));
  case BOTLIB_AI_GET_SECOND_GOAL:
    return botlib_export->ai.BotGetSecondGoal(args[1],
                                              (struct bot_goal_s *)VMA(2));
  case BOTLIB_AI_CHOOSE_LTG_ITEM:
    return botlib_export->ai.BotChooseLTGItem(args[1], (float *)VMA(2),
                                              (int *)VMA(3), args[4]);
  case BOTLIB_AI_CHOOSE_NBG_ITEM:
    return botlib_export->ai.BotChooseNBGItem(
        args[1], (float *)VMA(2), (int *)VMA(3), args[4],
        (struct bot_goal_s *)VMA(5), VMF(6));
  case BOTLIB_AI_TOUCHING_GOAL:
    return botlib_export->ai.BotTouchingGoal((float *)VMA(1),
                                             (struct bot_goal_s *)VMA(2));
  case BOTLIB_AI_ITEM_GOAL_IN_VIS_BUT_NOT_VISIBLE:
    return botlib_export->ai.BotItemGoalInVisButNotVisible(
        args[1], (float *)VMA(2), (float *)VMA(3), (struct bot_goal_s *)VMA(4));
  case BOTLIB_AI_GET_LEVEL_ITEM_GOAL:
    return botlib_export->ai.BotGetLevelItemGoal(args[1], (char *)VMA(2),
                                                 (struct bot_goal_s *)VMA(3));
  case BOTLIB_AI_GET_NEXT_CAMP_SPOT_GOAL:
    return botlib_export->ai.BotGetNextCampSpotGoal(
        args[1], (struct bot_goal_s *)VMA(2));
  case BOTLIB_AI_GET_MAP_LOCATION_GOAL:
    return botlib_export->ai.BotGetMapLocationGoal((char *)VMA(1),
                                                   (struct bot_goal_s *)VMA(2));
  ...
  case BOTLIB_AI_ADD_AVOID_SPOT:
    botlib_export->ai.BotAddAvoidSpot(args[1], (float *)VMA(2), VMF(3),
                                      args[4]);
    return 0;
  case BOTLIB_AI_MOVE_TO_GOAL:
    botlib_export->ai.BotMoveToGoal((struct bot_moveresult_s *)VMA(1), args[2],
                                    (struct bot_goal_s *)VMA(3), args[4]);
    return 0;
  case BOTLIB_AI_MOVE_IN_DIRECTION:
    return botlib_export->ai.BotMoveInDirection(args[1], (float *)VMA(2),
                                                VMF(3), args[4]);
  ...
  case BOTLIB_AI_MOVEMENT_VIEW_TARGET:
    return botlib_export->ai.BotMovementViewTarget(
        args[1], (struct bot_goal_s *)VMA(2), args[3], VMF(4), (float *)VMA(5));
  case BOTLIB_AI_PREDICT_VISIBLE_POSITION:
    return botlib_export->ai.BotPredictVisiblePosition(
        (float *)VMA(1), args[2], (struct bot_goal_s *)VMA(3), args[4],
        (float *)VMA(5));
  ...
  case BOTLIB_AI_INIT_MOVE_STATE:
    botlib_export->ai.BotInitMoveState(args[1],
                                       (struct bot_initmove_s *)VMA(2));
    return 0;

  case BOTLIB_AI_CHOOSE_BEST_FIGHT_WEAPON:
    return botlib_export->ai.BotChooseBestFightWeapon(args[1], (int *)VMA(2));
  case BOTLIB_AI_GET_WEAPON_INFO:
    botlib_export->ai.BotGetWeaponInfo(args[1], args[2],
                                       (struct weaponinfo_s *)VMA(3));
    return 0;
  ...
  case BOTLIB_AI_GENETIC_PARENTS_AND_CHILD_SELECTION:
    return botlib_export->ai.GeneticParentsAndChildSelection(
        args[1], (float *)VMA(2), (int *)VMA(3), (int *)VMA(4), (int *)VMA(5));
  ...
  case G_G2_SETMODELS:
    SV_G2API_SetGhoul2ModelIndexes(VMA(1), (qhandle_t *)VMA(2),
                                   (qhandle_t *)VMA(3));
    return 0;

  case G_G2_GETBOLT:
    return SV_G2API_GetBoltMatrix(
        VMA(1), args[2], args[3], (mdxaBone_t *)VMA(4), (const float *)VMA(5),
        (const float *)VMA(6), args[7], (qhandle_t *)VMA(8), (float *)VMA(9));

  case G_G2_GETBOLT_NOREC:
    return SV_G2API_GetBoltMatrix_NoReconstruct(
        VMA(1), args[2], args[3], (mdxaBone_t *)VMA(4), (const float *)VMA(5),
        (const float *)VMA(6), args[7], (qhandle_t *)VMA(8), (float *)VMA(9));

  case G_G2_GETBOLT_NOREC_NOROT:
    return SV_G2API_GetBoltMatrix_NoRecNoRot(
        VMA(1), args[2], args[3], (mdxaBone_t *)VMA(4), (const float *)VMA(5),
        (const float *)VMA(6), args[7], (qhandle_t *)VMA(8), (float *)VMA(9));
  ...
  case G_G2_INITGHOUL2MODEL:
    ...
    g_G2AllocServer = 1;
#endif
    return SV_G2API_InitGhoul2Model((void **)VMA(1), (const char *)VMA(2),
                                    args[3], (qhandle_t)args[4],
  ...
  case G_G2_ANGLEOVERRIDE:
    return SV_G2API_SetBoneAngles(
        VMA(1), args[2], (const char *)VMA(3), (float *)VMA(4), args[5],
        (const Eorientations)args[6], (const Eorientations)args[7],
        (const Eorientations)args[8], (qhandle_t *)VMA(9), args[10], args[11]);

  case G_G2_PLAYANIM:
    return SV_G2API_SetBoneAnim(VMA(1), args[2], (const char *)VMA(3), args[4],
                                args[5], args[6], VMF(7), args[8], VMF(9),
                                args[10]);

  case G_G2_GETBONEANIM:
    return SV_G2API_GetBoneAnim(
        VMA(1), (const char *)VMA(2), args[3], (float *)VMA(4), (int *)VMA(5),
  ...
  case G_G2_COLLISIONDETECT:
    SV_G2API_CollisionDetect((CollisionRecord_t *)VMA(1), VMA(2),
                             (const float *)VMA(3), (const float *)VMA(4),
                             args[5], args[6], (float *)VMA(7), (float *)VMA(8),
                             (float *)VMA(9), args[10], args[11], VMF(12));
    return 0;

  case G_G2_COLLISIONDETECTCACHE:
    SV_G2API_CollisionDetectCache(
        (CollisionRecord_t *)VMA(1), VMA(2), (const float *)VMA(3),
        (const float *)VMA(4), args[5], args[6], (float *)VMA(7),
        (float *)VMA(8), (float *)VMA(9), args[10], args[11], VMF(12));
    return 0;
  ...
  case G_G2_SETSURFACEONOFF:
    return SV_G2API_SetSurfaceOnOff(VMA(1), (const char *)VMA(2),
  ...
  case G_G2_GETSURFACERENDERSTATUS:
    return SV_G2API_GetSurfaceRenderStatus(VMA(1), args[2],
                                           (const char *)VMA(3));
  ...
  case G_G2_ANIMATEG2MODELS:
    SV_G2API_AnimateG2Models(VMA(1), args[2],
                             (sharedRagDollUpdateParams_t *)VMA(3));
    return 0;

  // additional ragdoll options -rww
  case G_G2_RAGPCJCONSTRAINT:
    return SV_G2API_RagPCJConstraint(VMA(1), (const char *)VMA(2),
                                     (float *)VMA(3), (float *)VMA(4));
  case G_G2_RAGPCJGRADIENTSPEED:
    return SV_G2API_RagPCJGradientSpeed(VMA(1), (const char *)VMA(2), VMF(3));
  case G_G2_RAGEFFECTORGOAL:
    return SV_G2API_RagEffectorGoal(VMA(1), (const char *)VMA(2),
                                    (float *)VMA(3));
  case G_G2_GETRAGBONEPOS:
    return SV_G2API_GetRagBonePos(VMA(1), (const char *)VMA(2), (float *)VMA(3),
                                  (float *)VMA(4), (float *)VMA(5),
                                  (float *)VMA(6));
  case G_G2_RAGEFFECTORKICK:
    return SV_G2API_RagEffectorKick(VMA(1), (const char *)VMA(2),
                                    (float *)VMA(3));
  case G_G2_RAGFORCESOLVE:
    return SV_G2API_RagForceSolve(VMA(1), (qboolean)args[2]);

  case G_G2_SETBONEIKSTATE:
    return SV_G2API_SetBoneIKState(VMA(1), args[2], (const char *)VMA(3),
                                   args[4],
                                   (sharedSetBoneIKStateParams_t *)VMA(5));
  case G_G2_IKMOVE:
...
void SV_InitGame(qboolean restart) {
  ...
  if (sv_legacyFixes->integer) {
    ...
    if (!gamename || !VALIDSTRING(gamename)) {
      ...
      if (!Q_stricmpn(gamename, "basejk", 6)) {
        if (gvm && !gvm->isLegacy)
          svs.servermod =
              SVMOD_OPENJK; // some OpenJK forks rename themselves to basejka
        else
        ...
        Com_DPrintf(S_COLOR_YELLOW
                    "Unsupported mod detected (%s) - some server engine "
  ...
  if (g_log && g_log->string && g_log->string[0] &&
      VALIDSTRING(g_log->string)) {
...
static void QDECL SV_GamePrint(const char *msg, ...) {
  va_list argptr;
  char text[1024];
  ...
  // RANKED NATIVE RANKED INTERCEPT FOR KILLS
  if (Q_stricmpn(text, "Kill: ", 6) == 0) {
    int killerId, victimId, mod;
    // Format: Kill: <killer> <victim> <mod>: <name> killed <name> by <MOD>
    if (sscanf(text, "Kill: %d %d %d:", &killerId, &victimId, &mod) == 3) {
      char wep[64] = "Unknown";
      const char *wMatch = strstr(text, " by ");
      if (wMatch) {
        Q_strncpyz(wep, wMatch + 4, sizeof(wep));
        char *nl = strchr(wep, '\n');
        if (nl) *nl = '\0';
      ...
      char vname[64] = "";
      const char *kMatch = strstr(text, " killed ");
      if (kMatch && wMatch && wMatch > kMatch) {
        int len = (int)(wMatch - (kMatch + 8));
        if (len > 0 && len < (int)sizeof(vname)) {
          memcpy(vname, kMatch + 8, len);
      ...
      const char *redMatch = strstr(text, "red:");
      const char *blueMatch = strstr(text, "blue:");
      if (redMatch && blueMatch) {
          int scoreRed = 0, scoreBlue = 0;
          sscanf(redMatch, "red:%d", &scoreRed);
          sscanf(blueMatch, "blue:%d", &scoreBlue);
          int winner = (scoreRed > scoreBlue) ? 1 : ((scoreBlue > scoreRed) ? 2 : 0);
          Com_Printf("[RANKED] End Game Team Scores Parsed (native): Red %d, Blue %d (Winner %d)\n", scoreRed, scoreBlue, winner);
          SV_Ranked_ProcessRoundEnd(winner);
      }
  }
...
void SV_UnbindGame(void) {
  ...
  // reset gamename cvar here so that it stays up to date in-case we switch out
  // mods or something.
