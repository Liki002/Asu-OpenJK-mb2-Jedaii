...
typedef struct {
  ...
  int minDeltaFrame;    // the first non-delta frame stored in the demo.  cannot
                        // delta against frames older than this
  fileHandle_t demofile;
  qboolean isBot;
  int botReliableAcknowledge; // for bots, need to maintain a separate
                              // reliableAcknowledge to record server messages
                              // into the demo file
} demoInfo_t;
...
typedef struct client_s {
  ...
  char guid[33];              // RANKED: OpenJK GUID
  ...
  // Wacky Mod Flags
  qboolean isUnlimited;

  int lastUserInfoChange; // if > svs.time && count > x, deny change -rww
  int lastUserInfoCount;  // allow a certain number of changes within a certain
                          // time period -rww
...
#ifdef DEDICATED
  qboolean disableDuelCull; // set for clients with "Duel see others" option set
                            // in cp_pluginDisable on JA+ servers
  qboolean jpPlugin;
  // kms...
  qboolean
      unfixPing; // set to true when client is estimated to have sent less than
                 // 60 packets in the last second, and falls back to baseJKA
                 // ping calculation when calculating it for this client

  int chatLogPolicySentTime;  // ugly hack of a workaround
  qboolean chatLogPolicySent; // set once client has been sent the "This server
                              // logs X chat messages" info, avoids sending
                              // message on map change
#endif
} client_t;
...
extern session_t sv_sessions[MAX_SESSIONS];
...
extern cvar_t *sv_sessionStrict; // RANKED
...
#define SVFIXES_DISABLE_MOVEMENT_EVENT_CHECKS                                  \
  (1 << 5) // backwards staff dfa exploit
...
#define SVFIXES_ALLOW_NEGATIVE_CALLVOTES                                       \
  (1 << 12) // negative fraglimit/timelimit callvotes
#define SVFIXES_DISABLE_SPEC_ALTFIRE_FOLLOWPREV                                \
  (1 << 12) // disables engine-side spectator alt-fire=followPrev feature
...
// Centerprint word-wrap helper (defined in sv_ccmds.cpp)
...
void SV_Trace(trace_t *results, const vec3_t start, const vec3_t mins,
              ...
              int contentmask, int capsule, int traceFlags, int useLod);
...
// sv_net_chan.c
//
void SV_Netchan_Transmit(client_t *client,
