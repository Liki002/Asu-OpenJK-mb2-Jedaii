...
cvar_t *sv_snapsMin; // minimum snapshots/sec a client can request, also limited
                     // by sv_snapsMax
cvar_t *sv_snapsMax; // maximum snapshots/sec a client can request, also limited
                     // by sv_fps
...
cvar_t *sv_botThinkFPS;
cvar_t *sv_sessionStrict = NULL; // RANKED
...
static inline bool
SV_MasterNeedsResolving(int server,
                        int time) { // refresh every so often regardless of if
                                    // the actual address was modified -rww
  if (g_lastResolveTime[server] > time) { // time flowed backwards?
    return true;
  }

  if ((time - g_lastResolveTime[server]) >
      NEW_RESOLVE_DURATION) { // it's time again
    return true;
  }
...
void SVC_Info(netadr_t from) {
  ...
  */
   ...
   * original Quake3 version which led to the Infostring bug discovered by Luigi
   * Auriemma. See http://aluigi.altervista.org/ for the advisory.
   */
  ...
  Info_SetValueForKey(infostring, "truejedi",
                      va("%i", Cvar_VariableIntegerValue("g_jediVmerc")));
  ...
  Info_SetValueForKey(
      infostring, "fdisable",
      va("%i", Cvar_VariableIntegerValue("g_forcePowerDisable")));
...
void SVC_RemoteCommand(netadr_t from, msg_t *msg) {
  qboolean valid;
  char remaining[1024];
  // TTimo - scaled down to accumulate, but not overflow anything network wise,
  // print wise etc. (OOB messages are the bottleneck here)
#define SV_OUTPUTBUF_LENGTH (1024 - 16)
  ...
  if (!strlen(sv_rconPassword->string)) {
    ...
    // since the cmd formatting can fuckup (amount of spaces), using a dumb step
    // by step parsing
    cmd_aux = Cmd_Cmd();
    cmd_aux += 4;
    while (cmd_aux[0] == ' ')
...
void SV_ConnectionlessPacket(netadr_t from, msg_t *msg) {
  ...
  if (sv_maxOOBRateIP->integer && com_dedicated->integer &&
    ...
    if (SVC_RateLimitAddress(from, burst, period, now)) {
      if (com_developer && com_developer->integer) {
        Com_Printf("SV_ConnectionlessPacket: Rate limit from %s exceeded, "
                   "dropping request\n",
                   NET_AdrToString(from));
      }
      return;
  ...
  if (dropped > 0 && lastMsg + 5000 < now) {
    Com_Printf(
        "SV_ConnectionlessPacket: Rate limit exceeded, dropped %d requests\n",
        dropped);
    dropped = 0;
    lastMsg = now;
  }
  ...
  for (i = 0; i < sv_maxclients->integer; i++) {
    ...
    if (!count) {
      ...
      if (sv_pingFix->integer &&
          cl->ping < 1) { // Botfilters assume that players with 0 ping are
                          // bots. So put the minimum ping for humans at 1. At
                          // least with the new ping calculation enabled.
        cl->ping = 1;
      }
    }
...
void SV_CheckCvars(void) {
  static int lastModHostname = -1, lastModFramerate = -1, lastModSnapsMin = -1,
             lastModSnapsMax = -1;
  static int lastModSnapsPolicy = -1, lastModRatePolicy = -1,
             lastModClientRate = -1;
  ...
  if (sv_clientRate->modificationCount != lastModClientRate ||
    ...
    if (sv_ratePolicy->integer == 1) {
      ...
      for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
        ...
        // running an internet public server, assume they don't need a rate
        // choke
  ...
  // check limits on client "snaps" value based on server framerate and snapshot
  // rate
    ...
    if (sv_snapsPolicy->integer == 1) {
      ...
      for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
        ...
        if (val != cl->snapshotMsec) {
          // Reset last sent snapshot so we avoid desync between server frame
      ...
      int maxSnaps =
          sv_snapsMax->integer > 0
              ...
              : sv_fps
                    ->integer; // can't produce more than sv_fps snapshots/sec,
  ...
  /*if (svs.servermod != SVMOD_UNKNOWN && svs.servermod != SVMOD_MBII) //on
  supported mod and isn't MB2 (logSync is necessary for RTV plugin)
  {
...
void SV_Frame(int msec) {
  ...
  if (svs.initialized && svs.gameStarted) {
    ...
    if (sv_hibernateTime->integer && !svs.hibernation.enabled && !humans) {
      int elapsed_time =
          Sys_Milliseconds() - svs.hibernation.lastTimeDisconnected;
