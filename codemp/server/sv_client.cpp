...
session_t sv_sessions[MAX_SESSIONS];
...
==================
*/
static void SV_GetClientGUID(client_t *cl) {
  char *val = Info_ValueForKey(cl->userinfo, "ja_guid");
  if (val && *val) {
    Q_strncpyz(cl->guid, val, sizeof(cl->guid));
  } else {
    cl->guid[0] = '\0';
  }
}
...
void SV_AddSession(client_t *cl) {
  ...
  for (i = 0; i < MAX_SESSIONS; i++) {
    if (sv_sessions[i].active) {
      // Check matching GUID first (Strongest Check)
      if (cl->guid[0] && sv_sessions[i].guid[0] &&
          !Q_stricmp(sv_sessions[i].guid, cl->guid) &&
          !Q_stricmp(sv_sessions[i].name, cl->name)) {
        sv_sessions[i].expiryTime = svs.time + SESSION_TIMEOUT;
        return;
      }
      // Fallback: Check IP + Name (For non-OpenJK clients)
      if (NET_CompareBaseAdr(sv_sessions[i].ip, cl->netchan.remoteAddress) &&
          !Q_stricmp(sv_sessions[i].name, cl->name)) {
        sv_sessions[i].expiryTime = svs.time + SESSION_TIMEOUT;
        return;
      }
  ...
  if (freeSlot != -1) {
    ...
    Q_strncpyz(sv_sessions[freeSlot].name, cl->name,
               sizeof(sv_sessions[freeSlot].name));
    // Store GUID
    if (cl->guid[0]) {
      Q_strncpyz(sv_sessions[freeSlot].guid, cl->guid,
                 sizeof(sv_sessions[freeSlot].guid));
    } else {
      sv_sessions[freeSlot].guid[0] = '\0';
    }
    sv_sessions[freeSlot].expiryTime = svs.time + SESSION_TIMEOUT;
    Com_Printf("Session added for %s (%s) [GUID: %s]. Expires at %d\n",
               NET_AdrToString(cl->netchan.remoteAddress), cl->name,
               cl->guid[0] ? cl->guid : "NONE",
               sv_sessions[freeSlot].expiryTime);
  }
}
...
qboolean SV_HasSession(client_t *cl) {
  ...
  for (i = 0; i < MAX_SESSIONS; i++) {
    if (sv_sessions[i].active) {
      ...
      // Primary Check: GUID Match
      if (cl->guid[0] && sv_sessions[i].guid[0] &&
          !Q_stricmp(sv_sessions[i].guid, cl->guid) &&
          !Q_stricmp(sv_sessions[i].name, cl->name)) {
        // RANKED: Security Fix - Strict Session Checking
        // If sv_sessionStrict is 1, we MUST also match the IP subnet to prevent
        // GUID spoofing/hijacking.
        if (sv_sessionStrict && sv_sessionStrict->integer) {
          if (NET_CompareBaseAdr(sv_sessions[i].ip,
                                 cl->netchan.remoteAddress)) {
            return qtrue; // GUID + IP Match -> Secure
          }
          // If strict is on and IP doesn't match, we ignore this session
          // (potentially a spoof attempt or dynamic IP drift too far)
          continue;
        } else {
          return qtrue; // Legacy/Lax mode: Trust GUID blindly
        }
      }

      // Fallback Check: IP Match (Only if both sides lack GUID or one does)
      // RANKED: Security Fix - If session has GUID, we MUST match it. No IP
      // fallback to prevent hijacking.
      if (!sv_sessions[i].guid[0] &&
          NET_CompareBaseAdr(sv_sessions[i].ip, cl->netchan.remoteAddress) &&
          !Q_stricmp(sv_sessions[i].name, cl->name)) {
        return qtrue;
      }
    }
...
void SV_GetChallenge(netadr_t from) {
  ...
  Cvar_VariableValue("ui_singlePlayerActive")) { return;
  }
  */
  ...
  // Create a unique challenge for this client without storing state on the
  // server
  ...
  NET_OutOfBandPrint(NS_SERVER, from, "challengeResponse %i %i", challenge,
                     clientChallenge);
}
...
void SV_DirectConnect(netadr_t from) {
  ...
  if (SV_IsBanned(&from, qfalse)) {
    if (sv_banmsg && sv_banmsg->string && sv_banmsg->string[0]) {
  ...
  for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
    ...
    /* This was preventing sv_reconnectlimit from working.  It seems like
       commenting this out has solved the problem.  HOwever, if there is a
       future problem then it could be this.
  ...
  for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
    ...
    if (NET_CompareBaseAdr(from, cl->netchan.remoteAddress) &&
      ...
      newcl = cl;
      // VVFIXME - both SOF2 and Wolf remove this call, claiming it blows away
      // the user's info disconnect the client from the game first so any flags
      // the player might have are dropped
...
// get the game a chance to reject this connection or modify the userinfo
#ifdef DEDICATED
  if (svs.servermod == SVMOD_JAPLUS &&
      Cvar_VariableIntegerValue("g_teamAutoJoin") &&
      !Cvar_VariableIntegerValue("g_gametype") &&
      Cvar_VariableIntegerValue("jp_teamLock") & (1 << 2)) // i guess?
  {
  ...
  // RANKED: Extract GUID for session persistence
  SV_GetClientGUID(newcl);

  // RANKED: Check for restored session
  if (SV_HasSession(newcl)) {
    Com_Printf("AUTH_RESTORE: %i %s [GUID: %s]\n", newcl - svs.clients,
               newcl->name, newcl->guid[0] ? newcl->guid : "NONE");
  }
}
...
void SV_DropClient(client_t *drop, const char *reason) {
  ...
  // RANKED NATIVE RANKED DB HOOK
  SV_Ranked_ClientDisconnect(drop);
...
void SV_SendClientChatLogPolicy(client_t *client) {
  ...
  if (svs.servermod == SVMOD_UNKNOWN ||
      (svs.servermod == SVMOD_MBII && client->chatLogPolicySent))
    return;

  if (svs.time - client->chatLogPolicySentTime <=
      5000) // don't send more than once every 5 seconds
    return;

  // SV_SendServerCommand(client, "print \"%sThis server logs %s chat
  // messages\n\"", S_COLOR_CYAN, com_logChat->integer == 1 ? "all public and
  // team" : "no");
  if (com_logChat->integer == 1) {
    SV_SendServerCommand(
        client,
        "print \"%scom_logChat is 1: This server does not log private messages "
        "(/tell) to protect player privacy\n\"",
        S_COLOR_CYAN);
  } else {
    SV_SendServerCommand(client,
                         "print \"%scom_logChat is 0: This server has a "
                         "no-logging policy to protect player privacy\n\"",
                         S_COLOR_CYAN);
  }
...
void SV_ClientEnterWorld(client_t *client, usercmd_t *cmd) {
  ...
  // RANKED: Store session for this client
  SV_AddSession(client);
...
void SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
  ...
  if (!cl->download) {
    ...
    if (!sv_allowDownload->integer || idPack || unreferenced ||
      ...
      if (unreferenced) {
        Com_Printf("clientDownload: %d : \"%s\" is not referenced and cannot "
                   "be downloaded.\n",
        ...
        if (missionPack) {
          Com_sprintf(errorMessage, sizeof(errorMessage),
                      ...
                      "The Team Arena mission pack can be found in your local "
                      "game store.",
                      cl->downloadName);
        } else {
          Com_sprintf(errorMessage, sizeof(errorMessage),
        ...
        // filesystem scheme?
        //	if the pk3 is referenced, it must have been found somewhere in
        // the filesystem
  ...
  while (cl->downloadCurrentBlock - cl->downloadClientBlock <
    ...
    if (!cl->downloadBlocks[curindex])
      cl->downloadBlocks[curindex] =
          (unsigned char *)Z_Malloc(MAX_DOWNLOAD_BLKSIZE, TAG_DOWNLOAD, qtrue);
  ...
  // client normal rate / snapshotMsec calculation
  rate = cl->rate;
...
static void SV_VerifyPaks_f(client_t *cl) {
  ...
  if (sv_pure->integer != 0) {
    ...
    bGood = qtrue;
    nChkSum1 = nChkSum2 = 0;
    // we run the game, so determine which cgame and ui the client "should" be
    // running
    ...
    while (bGood) {
      ...
      // store number to compare against (minus one cause the last is the number
      // of checksums)
      nClientPaks = i - 1;
      ...
      // check if the client has provided any pure checksums of pk3 files not
...
void SV_ClientCleanName(const char *in, char *out, int outSize) {
  ...
  for (; *in && outpos < outSize - 1; in++) {
    ...
    if (*(in + 1) && *(in + 1) != '\0' && *(in + 2) && *(in + 2) != '\0') {
      if (*in == ' ' && *(in + 1) == ' ' &&
          *(in + 2) == ' ') // don't allow more than 3 consecutive spaces
        continue;

      if (*in == '@' && *(in + 1) == '@' &&
          *(in + 2) == '@') // don't allow too many consecutive @ signs
        continue;
    }
...
void SV_UserinfoChanged(client_t *cl) {
  ...
  if (sv_legacyFixes->integer &&
      !(sv_legacyFixes->integer & SVFIXES_ALLOW_INVALID_PLAYER_NAMES) &&
      svs.servermod != SVMOD_JAPLUS && svs.servermod != SVMOD_MBII &&
  ...
  if (Sys_IsLANAddress(cl->netchan.remoteAddress) &&
    ...
    if (sv_ratePolicy->integer == 1) {
      ...
      i = atoi(val);
      if (!i) {
        i = sv_maxRate->integer;
      }
      i = Com_Clampi(1000, 2500000, i);
  ...
  // snaps command
  // Note: cl->snapshotMsec is also validated in sv_main.cpp -> SV_CheckCvars if
  // sv_fps, sv_snapsMin or sv_snapsMax is changed
  int minSnaps = sv_snapsMin->integer > 0
                     ? Com_Clampi(1, sv_snapsMax->integer, sv_snapsMin->integer)
                     : 1; // between 1 and sv_snapsMax ( 1 <-> 40 )
  int maxSnaps =
      sv_snapsMax->integer > 0
          ? Q_min(sv_fps->integer, sv_snapsMax->integer)
          : sv_fps->integer; // can't produce more than sv_fps snapshots/sec,
                             // but can send less than sv_fps snapshots/sec
  ...
  if (sv_fps && sv_fps->integer && sv_snapsPolicy->integer == 1) {
    ...
    if (i != cl->snapshotMsec) {
      // Reset next snapshot so we avoid desync between server frame time and
      // snapshot send time
      cl->nextSnapshotTime = -1;
      cl->snapshotMsec = i;
    }
  ...
  if (val) {
    ...
    if (sv_legacyFixes->integer &&
        !(sv_legacyFixes->integer & SVFIXES_ALLOW_BROKEN_MODELS) &&
        svs.servermod != SVMOD_MBII) {
      len = (int)strlen(val);
      qboolean badModel = qfalse;

      if (!Q_stricmpn(val, "jedi_", len) &&
          (!Q_stricmpn(val, "jedi_/red", len) ||
  ...
  if (sv_legacyFixes->integer &&
      !(sv_legacyFixes->integer & SVFIXES_ALLOW_INVALID_FORCEPOWERS)) {
    char forcePowers[30];
    Q_strncpyz(forcePowers, Info_ValueForKey(cl->userinfo, "forcepowers"),
               sizeof(forcePowers));
    ...
    if (len >= 22 && len <= 24) {
      ...
      for (int i = 0; i < len; i++) {
        if (forcePowers[i] != '-' &&
            (forcePowers[i] < '0' || forcePowers[i] > '9')) {
          badForce = qtrue;
          break;
        }
  ...
  if (svs.servermod == SVMOD_JAPLUS ||
      svs.servermod ==
          SVMOD_JAPRO) { // allow JA+ clients to configure duel isolation on JA+
                         // servers using /pluginDisable
    val = Info_ValueForKey(cl->userinfo, "cjp_client");
    if (val &&
        strlen(val) >= 3) { // make sure they have some version of the plugin
      cl->jpPlugin = qtrue;
      val = Info_ValueForKey(cl->userinfo, "cp_pluginDisable");
      if (svs.servermod == SVMOD_JAPRO &&
          (atoi(val) & (1 << 1))) { // JAPRO_PLUGIN_DUELSEEOTHERS
  ...
  // Update backend Ranked Database with their dynamic coloring/name immediately
  SV_Ranked_UpdateDisplayName(cl - svs.clients, cl->name);

  // Update team in ranked state from fresh userinfo
  {
    int clientNum = cl - svs.clients;
    rankedMatchState_t *mState = &sv_rankedPlayers[clientNum];
    const char *teamStr = Info_ValueForKey(cl->userinfo, "team");
    if (teamStr && *teamStr) {
      int parsedTeam = 0;
      ...
      if (parsedTeam > 0) {
        ...
        Com_Printf("[RANKED] Team updated for client %d: %d\n", clientNum, parsedTeam);
      }
    }
  }
}
...
static void SV_UpdateUserinfo_f(client_t *cl) {
  ...
  if (cl->lastUserInfoChange > svs.time) {
    ...
    if (cl->lastUserInfoCount >= INFO_CHANGE_MAX_COUNT) {
      //	SV_SendServerCommand(cl, "print \"Warning: Too many info
      // changes, last info ignored\n\"\n"); 	SV_SendServerCommand(cl, "print
      //\"@@@TOO_MANY_INFO\n\"\n");
...
void SV_ExecuteClientCommand(client_t *cl, const char *s, qboolean clientOK) {
  ...
  // --- RANKED NATIVE RANKED SYSTEM HOOKS ---
  if (!Q_stricmp(Cmd_Argv(0), "login") || !Q_stricmp(Cmd_Argv(0), "register")) {
    if (Cmd_Argc() < 3) {
      SV_SendServerCommand(cl,
                           va("print \"Usage: %s <username> <password>\n\"", Cmd_Argv(0)));
      return;
    }
    // Call Native Database
    SV_Ranked_LoginOrRegister(cl, Cmd_Argv(1), Cmd_Argv(2));
    return;
  }
  if (!Q_stricmp(Cmd_Argv(0), "logout")) {
    SV_Ranked_Logout(cl);
    return;
  }
  ...
  if (!Q_stricmp(Cmd_Argv(0), "duelaccept")) {
      Com_Printf("[RANKED] Engine duelaccept detected from %s\n", cl->name);
      // We don't call DuelStart here because the engine/game will trigger the actual duel state,
      // and sv_gameapi.cpp already has a "ranked_duel_start" hook which is the authoritative source.
      // However, we can log it for debugging.
  }
  // ----------------------------------------
  ...
  if (!Q_stricmpn(cmd, "jkaDST_", 7) &&
      cl->netchan.remoteAddress.type !=
          NA_LOOPBACK) { // typo'd a mistyped DST setting
  ...
  if (!Q_stricmpn(cmd, "say", 3) || !Q_stricmpn(cmd, "say_team", 8) ||
  ...
  if (sv_legacyFixes->integer && svs.servermod != SVMOD_MBII) {
    if (!(sv_legacyFixes->integer & SVFIXES_DISABLE_GC_CRASHFIX) &&
        !Q_stricmpn(cmd, "gc", 2) && atoi(arg1) >= sv_maxclients->integer)
      clientOK = qfalse;

    if (!(sv_legacyFixes->integer & SVFIXES_DISABLE_NPC_CRASHFIX) &&
        svs.servermod != SVMOD_JAPRO && !Q_stricmpn(cmd, "npc", 3) &&
        !Q_stricmpn(arg1, "spawn", 5) &&
        (!Q_stricmpn(arg2, "ragnos", 6) || !Q_stricmpn(arg2, "saber_droid", 6)))
      clientOK = qfalse;

    // Fix: team crash
    if (!(sv_legacyFixes->integer & SVFIXES_DISABLE_TEAM_CRASHFIX) &&
        !Q_stricmpn(cmd, "team", 4) &&
        (!Q_stricmpn(arg1, "follow1", 7) || !Q_stricmpn(arg1, "follow2", 7)))
      clientOK = qfalse;

    // Disable: callteamvote, useless in basejka and can lead to a bugged UI on
    // custom client
    if (!(sv_legacyFixes->integer & SVFIXES_ALLOW_CALLTEAMVOTE) &&
        svs.servermod == SVMOD_BASEJKA && !Q_stricmpn(cmd, "callteamvote", 12))
      clientOK = qfalse;

    // Fix: callvote fraglimit/timelimit with negative value
    if (!(sv_legacyFixes->integer & SVFIXES_ALLOW_NEGATIVE_CALLVOTES) &&
        svs.servermod == SVMOD_BASEJKA && !Q_stricmpn(cmd, "callvote", 8) &&
        (!Q_stricmpn(arg1, "fraglimit", 9) ||
         !Q_stricmpn(arg1, "timelimit", 9)) &&
        atoi(arg2) < 0)
      clientOK = qfalse;
  }
...
static qboolean SV_ClientCommand(client_t *cl, msg_t *msg) {
  ...
  Com_sprintf(cl->lastClientCommandString, sizeof(cl->lastClientCommandString),
              "%s", s);
...
void SV_ClientThink(client_t *cl, usercmd_t *cmd) {
  ...
  if (sv_legacyFixes->integer &&
      !(sv_legacyFixes->integer & SVFIXES_DISABLE_SPEC_ALTFIRE_FOLLOWPREV) &&
      (svs.servermod == SVMOD_BASEJKA || svs.servermod == SVMOD_JAPLUS) && ps &&
      (ps->pm_flags & PMF_FOLLOW) && (cmd->buttons & BUTTON_ALT_ATTACK) &&
      !(cmd->buttons & BUTTON_ATTACK) &&
      !(cl->lastUsercmd.buttons &
        BUTTON_ALT_ATTACK)) { // allow alt attack to go back one player in
                              // spectator
  ...
  if (cl->lastUserInfoCount >= INFO_CHANGE_MAX_COUNT &&
      ...
      cl->userinfoPostponed[0]) { // Update postponed userinfo changes now
...
static void SV_UserMove(client_t *cl, msg_t *msg, qboolean delta) {
  ...
  for (i = 0; i < cmdCount; i++) {
    ...
    if (sv_legacyFixes->integer) {
      if (!(sv_legacyFixes->integer & SVFIXES_ALLOW_INVALID_FORCESEL) &&
          (cmd->forcesel == FP_LEVITATION ||
           cmd->forcesel >=
               NUM_FORCE_POWERS)) { // block "charge jump" and other nonsense
        cmd->forcesel = 0xFFu;
      }

      if (!(sv_legacyFixes->integer &
            SVFIXES_ALLOW_INVALID_VIEWANGLES)) { // affects speed calculation
        cmd->angles[ROLL] = 0;
      }
    }
    ...
    if (sv_strictPacketTimestamp->integer && cl->state == CS_ACTIVE) {
      if (cmd->serverTime < (sv.time - 1000)) {
        ...
        if (lastWarnTime < sv.time - 5000) {
          ...
          Com_DPrintf("client %i(%i) serverTime too low (%i < %i: %.2fs)\n",
                      cl - svs.clients, cl->state, cmd->serverTime,
                      sv.time - 1000,
        ...
        if (lastWarnTime < sv.time - 5000) {
          lastWarnTime = sv.time;
          Com_DPrintf("client %i:%i serverTime in future (%i > %i: %.2fs)\n",
                      cl - svs.clients, cl->state, cmd->serverTime,
                      sv.time + 200,
  ...
  // With sv_pingFix enabled we store the time of the first acknowledge, instead
  // of the last. And we use a time value that is not limited by sv_fps.
  if (!fixPing ||
      cl->frames[cl->messageAcknowledge & PACKET_MASK].messageAcked == -1)
    cl->frames[cl->messageAcknowledge & PACKET_MASK].messageAcked =
        (fixPing ? Sys_Milliseconds() : svs.time);

  // TTimo
  // catch the no-cp-yet situation before SV_ClientEnterWorld
  // if CS_ACTIVE, then it's time to trigger a new gamestate emission
  // if not, then we are getting remaining parasite usermove commands, which we
  // should ignore
...
#ifdef DEDICATED
  if (lastServerTime <=
      0) { // lastServerTime is always 0 if client is sending 1 cmd per packet
    lastServerTime = firstServerTime;
  }

  if (sv_pingFix->integer == 2 && oldServerTime > 0 && firstServerTime > 0 &&
      lastServerTime > 0) {
    ...
    if (packetDelta > 0) {
      ...
      previousPacketDeltas[clientNum][previousPacketDeltasIndex[clientNum] %
                                      PACKET_BACKUP] = packetDelta;
      ...
      // allowing for some leeway but is supposed to use old ping calculation if
      // their packet rate is less than 55-60 cl->unfixPing =
      // (qboolean)(packetDelta > 20);// serverFrameMsec)
      ...
      if (cl->unfixPing && com_developer->integer > 3) { // debug spew...
        ...
        Com_sprintf(buf, sizeof(buf),
                    S_COLOR_MAGENTA
                    "Packet delta too low -  using old ping calc on client %i "
                    "(delta %i average %i count %i)\n",
