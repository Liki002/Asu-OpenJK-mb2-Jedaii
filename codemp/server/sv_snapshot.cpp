...
static void SV_WriteSnapshotToClient(client_t *client, msg_t *msg) {
  ...
  // bots never acknowledge, but it doesn't matter since the only use case is
  // for serverside demos in which case we can delta against the very last
  // message every time
  ...
  if (deltaMessage <= 0 || client->state != CS_ACTIVE) {
    ...
    // demo is waiting for a non-delta-compressed frame for this client, so
    // don't delta compress
    oldframe = NULL;
    lastframe = 0;
  } else if (client->demo.minDeltaFrame > deltaMessage) {
    // we saved a non-delta frame to the demo and sent it to the client, but the
    // client didn't ack it we can't delta against an old frame that's not in
    // the demo without breaking the demo.  so send non-delta frames until the
    // client acks.
    oldframe = NULL;
    lastframe = 0;
  } else {
  ...
  if (oldframe) {
    ...
    MSG_WriteDeltaPlayerstate(msg, &oldframe->ps, &frame->ps,
                              frame->pDeltaOneBit, frame->pDeltaNumBit);
#else
    MSG_WriteDeltaPlayerstate(msg, &oldframe->ps, &frame->ps);
#endif
    if (frame->ps.m_iVehicleNum) { // then write the vehicle's playerstate too
      if (!oldframe->ps
               .m_iVehicleNum) { // if last frame didn't have vehicle, then the
                                 // old vps isn't gonna delta properly (because
                                 // our vps on the client could be anything)
#ifdef _ONEBIT_COMBO
...
#ifdef _ONEBIT_COMBO
        MSG_WriteDeltaPlayerstate(msg, &oldframe->vps, &frame->vps,
                                  frame->pDeltaOneBitVeh,
                                  frame->pDeltaNumBitVeh, qtrue);
#else
...
#ifdef DEDICATED
  if (!client->chatLogPolicySent ||
      (svs.servermod != SVMOD_UNKNOWN && svs.servermod != SVMOD_MBII && frame &&
       oldframe &&
       ((oldframe->ps.pm_flags & PMF_FOLLOW) ||
        oldframe->ps.pm_type == PM_SPECTATOR) &&
       !(frame->ps.pm_flags & PMF_FOLLOW) &&
       frame->ps.pm_type !=
           PM_SPECTATOR)) { // either hasn't been sent the message or they just
                            // switched from spectators
...
#endif
{
  ...
  for (e = 0; e < sv.num_entities; e++) {
    ...
    if (ent->s.eFlags &
        EF_PERMANENT) { // he's permanent, so don't send him down!
      continue;
    }
...
#ifdef DEDICATED
    if (!skipDuelCull &&
        DuelCull(SV_GentityNum(frame->ps.clientNum), ent) == 1) {
      continue;
    }
#endif

    if (ent->s.eType >= ET_EVENTS && sv_legacyFixes->integer &&
        !(sv_legacyFixes->integer & SVFIXES_DISABLE_MOVEMENT_EVENT_CHECKS) &&
        svs.servermod < SVMOD_JAPRO && svs.servermod != SVMOD_UNKNOWN &&
        svs.servermod !=
            SVMOD_MBII) // only check event types on known mods, to avoid
                        // modified eTypes/event enum conflicts
    {
      int eventNum = (ent->s.eType - ET_EVENTS) & ~EV_EVENT_BITS;

      if (eventNum == EV_JUMP || eventNum == EV_FALL ||
          eventNum ==
              EV_FOOTSTEP) { // block these movement-triggered event entities,
                             // these should always be on a player
        continue;
      }

      if ((eventNum == EV_PLAY_EFFECT || eventNum == EV_PLAY_EFFECT_ID) &&
          (ent->s.eventParm >= EFFECT_WATER_SPLASH &&
           ent->s.eventParm <= EFFECT_LANDING_GRAVEL)) // all landing effects
      {
        effectCount++;
    ...
    // entities can request not to be sent to certain clients (NOTE: always send
    // to ourselves)
    if (e != frame->ps.clientNum && (ent->r.svFlags & SVF_BROADCASTCLIENTS) &&
        !(ent->r.broadcastClients[frame->ps.clientNum / 32] &
          (1 << (frame->ps.clientNum % 32)))) {
      continue;
    }
    // broadcast entities are always sent, and so is the main player so we don't
    // see noclip weirdness
    ...
    if (ent->s.isPortalEnt) { // rww - portal entities are always sent as well
      SV_AddEntToSnapshot(svEnt, ent, eNums);
      continue;
    }
    ...
    if (g_svCullDist != -1.0f) { // do a distance cull check
      ...
      if (length - radius >= g_svCullDist) { // then don't add it
        continue;
      }
    }
...
static void SV_BuildClientSnapshot(client_t *client) {
  ...
  if (ps->m_iVehicleNum) { // get the vehicle's playerstate too then
    ...
    if (veh &&
        veh->playerState) { // Now VMA it and we've got ourselves a playerState
      playerState_t *vps =
          ((playerState_t *)VM_ArgPtr((intptr_t)veh->playerState));
...
static int SV_RateMsec(client_t *client, int messageSize) {
  ...
  // Reduced overhead calculation for MB2 smoothness at 50k rate - "The other
  // guy's fix"
...
void SV_SendMessageToClient(msg_t *msg, client_t *client) {
  ...
  // save the message to demo.  this must happen before sending over network as
  // that encodes the backing databuf
  ...
  if (client->netchan.remoteAddress.type == NA_LOOPBACK ||
    ...
    client->nextSnapshotTime =
        svs.time + ((int)(1000.0 / sv_fps->integer * com_timescale->value));
    return;
  }
  ...
  if (client->state != CS_ACTIVE) {
    ...
    if (!*client->downloadName &&
        client->nextSnapshotTime <
            svs.time + ((int)(1000.0 * com_timescale->value))) {
      client->nextSnapshotTime =
          svs.time + ((int)(1000 * com_timescale->value));
    }
  }
}
...
void SV_SendClientSnapshot(client_t *client) {
  ...
  if (!client->sentGamedir) { // rww - if this is the case then make sure there
                              // is an svc_setgame sent before this snap
    int i = 0;
    ...
    while (client->state && client->netchan.unsentFragments) {
      // send additional message fragments if the last message
      // was too large to send at once
      Com_Printf("[ISM]SV_SendClientGameState() [1] for %s, writing out old "
                 "fragments\n",
                 client->name);
      SV_Netchan_TransmitNextFragment(&client->netchan);
    }
