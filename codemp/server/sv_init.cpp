...
void SV_Startup(void) {
  ...
  svs.clients = (struct client_s *)Z_Malloc(
      sizeof(client_t) * sv_maxclients->integer, TAG_CLIENTS, qtrue);
...
void SV_ChangeMaxClients(void) {
  ...
  oldClients =
      (struct client_s *)Hunk_AllocateTempMemory(count * sizeof(client_t));
  ...
  svs.clients = (struct client_s *)Z_Malloc(
      sv_maxclients->integer * sizeof(client_t), TAG_CLIENTS, qtrue);
...
void SV_SpawnServer(char *server, qboolean killBots,
  ...
  */
  ...
  // the server also print some status stuff
  CL_MapLoading();
  ...
  */
  ...
  //	svs.snapshotEntities = (struct entityState_s *)Hunk_Alloc(
  ...
  // we CAN afford to do this here, since we know the STL vectors in Ghoul2 are
  // empty
  ...
  */
  ...
  if (sv_pure->integer) {
    ...
    // if a dedicated pure server we need to touch the cgame because it could be
    // in a seperate pk3 file and the client will need to load the latest
    // cgame.qvm
    if (com_dedicated->integer) {
      SV_TouchCGame();
    }
  ...
  // RANKED: Refresh active mode (Duel vs Open) for the new map
  SV_Ranked_SaveConfig();
  SV_Ranked_MapChange();
  Com_Printf("[RANKED] Mode set to: %s\n", SV_Ranked_GetActiveMode());
  ...
  Hunk_SetMark CL_StartHunkUsers();
  }
  */
  ...
  for (client_t *client = svs.clients;
    ...
    // cannot do this above where it says it will because mapname is not set at
    // that time
...
static void CM_SetUsingCache(qboolean usingCache) {
  gbUsingCachedMapDataRightNow = usingCache;
}
...
static IHeapAllocator *GetG2VertSpaceServer(void) { return G2VertSpaceServer; }
...
void SV_Init(void) {
  ...
  Cvar_Get("sv_ranked_enabled", "1", CVAR_ARCHIVE,
            "Enable (1) or disable (0) the entire Jedaii Ranked system. "
            "Can be toggled at runtime via RCON without restarting the server.");
  ...
  // Get these to establish them and to make sure they have a default before the
  // menus decide to stomp them.
  ...
  sv_ratePolicy = Cvar_Get("sv_ratePolicy", "1", CVAR_ARCHIVE_ND,
                           "Determines which policy of enforcement is used for "
                           "client's \"rate\" cvar");
  Cvar_CheckRange(sv_ratePolicy, 1, 2, qtrue);
  sv_clientRate = Cvar_Get("sv_clientRate", "90000", CVAR_ARCHIVE_ND);
  sv_minRate =
      Cvar_Get("sv_minRate", "0", CVAR_ARCHIVE_ND | CVAR_SERVERINFO,
               "Min bandwidth rate allowed on server. Use 0 for unlimited.");
  sv_maxRate =
      Cvar_Get("sv_maxRate", "0", CVAR_ARCHIVE_ND | CVAR_SERVERINFO,
  ...
  sv_banmsg = Cvar_Get("sv_banmsg", "", CVAR_ARCHIVE, "Custom message displayed to banned users on connection attempt");
  sv_floodProtect =
      Cvar_Get("sv_floodProtect", "1", CVAR_ARCHIVE | CVAR_SERVERINFO,
               "Protect against flooding of server commands");
  sv_floodProtectSlow = Cvar_Get(
      "sv_floodProtectSlow", "1", CVAR_ARCHIVE | CVAR_SERVERINFO,
      "Use original method of delaying commands with flood protection");
  // systeminfo
  ...
  sv_snapsMin =
      Cvar_Get("sv_snapsMin", "0", CVAR_ARCHIVE_ND); // 1 <=> sv_snapsMax
  sv_snapsMax =
      Cvar_Get("sv_snapsMax", "0", CVAR_ARCHIVE_ND); // sv_snapsMin <=> sv_fps
  sv_snapsPolicy = Cvar_Get("sv_snapsPolicy", "2", CVAR_ARCHIVE_ND,
                            "Determines which policy of enforcement is used "
                            "for client's \"snaps\" cvar");
  Cvar_CheckRange(sv_snapsPolicy, 0, 2, qtrue);
  sv_fps = Cvar_Get("sv_fps", "50", CVAR_ARCHIVE | CVAR_SERVERINFO,
  ...
  sv_allowDownload =
      Cvar_Get("sv_allowDownload", "0", CVAR_SERVERINFO,
  ...
  sv_master[1] =
      Cvar_Get("sv_master2", JKHUB_MASTER_SERVER_NAME, CVAR_PROTECTED);
  ...
  sv_autoDemo = Cvar_Get("sv_autoDemo", "0", CVAR_ARCHIVE_ND | CVAR_SERVERINFO,
...
#ifndef DEDICATED // Default this to off on client to avoid potential mod
                  // compatibility issues.
  sv_legacyFixes = Cvar_Get("sv_legacyFixes", "0", CVAR_ARCHIVE);
#else
  sv_legacyFixes = Cvar_Get("sv_legacyFixes", "1", CVAR_ARCHIVE);
#endif
  sv_strictPacketTimestamp =
      Cvar_Get("sv_strictPacketTimestamp", "1", CVAR_NONE, "");
  ...
  sv_hibernateTime =
      Cvar_Get("sv_hibernateTime", "20000", CVAR_ARCHIVE_ND,
               "Time after which server will enter hibernation mode");
  sv_hibernateFPS = Cvar_Get("sv_hibernateFPS", "2", CVAR_ARCHIVE_ND,
                             "FPS during hibernation mode");
  Cvar_CheckRange(sv_hibernateFPS, 1, 125, qtrue);

  sv_botThinkFPS = Cvar_Get("sv_botThinkFPS", "20", CVAR_ARCHIVE_ND,
                            "Bot thinking rate in frames per second");

  sv_maxOOBRate = Cvar_Get("sv_maxOOBRate", "1000", CVAR_ARCHIVE,
                           "Maximum rate of handling incoming server commands");
  sv_maxOOBRateIP = Cvar_Get(
      "sv_maxOOBRateIP", "1", CVAR_ARCHIVE,
      "Maximum rate of handling incoming server commands per IP address");
  sv_autoWhitelist =
      Cvar_Get("sv_autoWhitelist", "1", CVAR_ARCHIVE,
               "Save player IPs to allow them using server during DOS attack");

#ifdef DEDICATED
  sv_antiDST =
      Cvar_Get("sv_antiDST", "1", CVAR_NONE,
               "Attempts to detect and kick players injecting or using DST");
  sv_pingFix = Cvar_Get("sv_pingFix", "2", CVAR_ARCHIVE_ND,
                        "Improved scoreboard client ping calculation - 1: "
                        "always use new ping calculation - 2: fall back to old "
                        "method if client's packet rate is less than 60");

  // Used to control/force certain settings at all times
  // sv_g_logSync = Cvar_Get("g_logsync", "0", CVAR_NONE, "");
#else
  sv_pingFix = Cvar_Get("sv_pingFix", "1", CVAR_ARCHIVE_ND,
  ...
  // create a heap for Ghoul2 to use for game side model vertex transforms used
...
void SV_Shutdown(char *finalmsg) {
  ...
  // Save all player stats and release the JSON memory tree before engine dies
  SV_Ranked_Shutdown();
  ...
  */
  ...
  // free current level
  SV_ClearServer();
  CM_ClearMap(); // jfm: add a clear here since it's commented out in
                 // clearServer.  This prevents crashing cmShaderTable on exit.
