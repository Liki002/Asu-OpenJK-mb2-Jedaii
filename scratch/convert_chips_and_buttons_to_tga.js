...
const inputDir = path.join(__dirname, 'NewAssets', 'chips and buttons');
const chipsDir = path.join(__dirname, '..', 'pk3_ranked_rpg', 'gfx', 'rpg_hud', 'chips');
const buttonsDir = path.join(__dirname, '..', 'pk3_ranked_rpg', 'gfx', 'rpg_hud', 'buttons');

if (!fs.existsSync(chipsDir)) fs.mkdirSync(chipsDir, { recursive: true });
...
for (const file of files) {
    ...
    const isChip = file.startsWith('chip_');
    const targetDir = isChip ? chipsDir : buttonsDir;
    const tgaPath = path.join(targetDir, tgaFilename);
    ...
    try {
        ...
        if (isChip) countChips++;
        else countButtons++;
        console.log(`Converted: ${file} -> ${path.relative(path.join(__dirname, '..'), tgaPath)} (${width}x${height})`);
    } catch (err) {
        console.error(`Failed to convert ${file}:`, err.message);
    }
}

console.log(`Successfully converted ${countChips} chips and ${countButtons} buttons to 32-bit TGAs.`);
